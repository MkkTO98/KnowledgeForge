# Folder Summary: docs

## Purpose
This folder is part of the ProjectForge file-backed operating system for `docs`.

## Contains
<!-- PROJECTFORGE:BEGIN-CONTAINS -->
- `.gitkeep`
- `AUTONOMY_LEVELS.md`
- `BRANCH_STRATEGY.md`
- `architecture.md`
- `campaign34_wdi_denmark_population_statistical_summary.md`
- `campaign35_bounded_replication_spec.md`
- `campaign35_production_value_assessment.md`
- `campaign35_wdi_nordic_exports_statistical_summary.md`
- `campaign36_bounded_correlation_spec.md`
- `campaign36_production_value_assessment.md`
- `campaign37_production_value_assessment.md`
- `campaign38_production_value_assessment.md`
- `campaign39_production_value_assessment.md`
- `correlation_knowledge_boundary.md`
- `doctrine_review_triggers.md`
- `evidence_source_evaluation_specification.md`
- `governed_vocabularies.md`
- `interfaces.md`
- `invariants.md`
- `knowledge_acceptance_criteria.md`
- `knowledge_evolution_change_report_contract.md`
- `knowledge_package_contract.md`
- `knowledge_repository_architecture.md`
- `knowledge_repository_persistence_specification.md`
- `local_ai_role_boundary.md`
- `model_routing_policy.md`
- `open_questions.md`
- `operational_autonomy_declaration.md`
- `pearson_correlation_method_v1.md`
- `postgresql_operational_repository.md`
- `postgresql_projection_config.example`
- `principles.md`
- `production_campaign_roadmap.md`
- `production_doctrine.md`
- `production_evolution_log.md`
- `production_quality_assessment.md`
- `production_value_scorecard.md`
- `provenance_fingerprinting.md`
- `reproducible_knowledge_generation_architecture.md`
- `roadmap.md`
- `standard_operational_loop.md`
- `statistical_summary_measure_utility_rules.md`
- `statistical_summary_method_v2.md`
- `statistical_summary_pilot_evaluation_gate.md`
- `validation_framework_v1.md`
- `validator_taxonomy.md`
- `vertical_slice_0_implementation_design.md`
- `wdi_observation_evidence_fixture.md`
- `wdi_unit_resolution.md`
<!-- PROJECTFORGE:END-CONTAINS -->

## Active Work
- `production_campaign_roadmap.md` records completed non-WDI multi-source disagreement planning gate: no candidate selected; reopen only after a source-independent metadata-complete two-source evidence bundle exists.

## Needs Attention
- No source acquisition, ingestion implementation, package production, PostgreSQL mutation, Relationship Export output mutation, or doctrine/architecture change is authorized by the completed planning gate.
