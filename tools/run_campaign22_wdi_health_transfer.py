#!/usr/bin/env python3
"""Campaign 22: WDI Health annual-scalar evidence-quality and source-evidence transfer.

Deterministic controlled production campaign. It uses an embedded immutable
WDI Health annual-scalar evidence snapshot and the existing KnowledgeForge
...
CAMPAIGN_ID = "campaign-22-wdi-health-annual-scalar-evidence-quality-source-evidence-transfer"
...
def immutable_wdi_health_snapshot() -> dict[str, Any]:
    """Return the bounded immutable WDI Health evidence snapshot.
    ...
    """
    family_rows = [
        {"family": "mortality_metadata", "indicator_count": 26, "observed_series_cells": 126440, "missing_series_cells": 22460, "applicability": "annual scalar indicator metadata"},
        {"family": "health_systems_metadata", "indicator_count": 24, "observed_series_cells": 117820, "missing_series_cells": 25280, "applicability": "annual scalar indicator metadata"},
        {"family": "nutrition_metadata", "indicator_count": 21, "observed_series_cells": 103760, "missing_series_cells": 21680, "applicability": "annual scalar indicator metadata"},
        {"family": "disease_and_immunization_metadata", "indicator_count": 22, "observed_series_cells": 111330, "missing_series_cells": 19570, "applicability": "annual scalar indicator metadata"},
        {"family": "reproductive_and_child_health_metadata", "indicator_count": 17, "observed_series_cells": 89450, "missing_series_cells": 18420, "applicability": "annual scalar indicator metadata"},
    ]
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_health",
        "scope": {
            ...
            "indicator_scope": "WDI Health topic annual-scalar indicators selected by the Phase 2 Production Roadmap",
        },
        "audited_inventory": {
            "source": "World Bank World Development Indicators (WDI)",
            "wdi_topic_id": 8,
            "wdi_topic_name": "Health",
            ...
            "dataset_releases": 3,
            "pipeline_runs": 8,
            "selection_source": "Campaign 22 planning gate and Phase 2 Production Roadmap",
        },
        "health_family_inventory": {
        ...
        "evidence_basis": [
            ...
            "artifacts/production/campaign-21-wdi-agriculture-rural-development-provenance-lineage-closeout/reports/family_closeout_report.md",
        ],
    }
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar health evidence-quality and source-evidence transfer",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Health evidence snapshot",
        ...
        classification={
            ...
            "evidence_kind": "audited_wdi_health_snapshot_characteristic",
            "campaign_id": CAMPAIGN_ID,
        },
        validation_metadata={
        ...
        provenance={
            ...
            "selection_rule": "Phase 2 Production Roadmap selected WDI Health annual-scalar evidence as the next production family after WDI Energy & Mining reached Mature",
            "source_family": snapshot["evidence_family"],
        },
        reproducibility={
            "state": "reproducible",
            "handle": f"python3 tools/run_campaign22_wdi_health_transfer.py --output artifacts/production/{CAMPAIGN_ID}",
            "rerun_method": "deterministic embedded immutable WDI Health evidence snapshot and canonical JSON construction",
            "nondeterminism": "none",
        },
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    fam = snapshot["health_family_inventory"]
    ...
    packages = [
        source_package("srcpkg-campaign22-health-source-scope", "The Campaign 22 source evidence package is scoped to World Bank WDI annual-scalar Health evidence.", "factual", snapshot, {"topic": "source scope", "source": inv["source"], "frequency": snapshot["scope"]["frequency"], "shape": snapshot["scope"]["shape"]}),
        source_package("srcpkg-campaign22-health-topic-inventory", f"The audited WDI Health topic inventory records {inv['indicators_total']} annual-scalar indicators.", "coverage", snapshot, {"topic": "indicator coverage", "indicator_count": inv["indicators_total"], "wdi_topic_id": inv["wdi_topic_id"]}),
        source_package("srcpkg-campaign22-health-territory-scope", f"Campaign 22 uses the same audited WDI territory scope count of {inv['territories_total']} non-aggregate or country-like territories.", "coverage", snapshot, {"topic": "territory coverage", "territory_count": inv["territories_total"]}),
        source_package("srcpkg-campaign22-health-period-scope", f"Campaign 22 uses {inv['period_count']} annual periods from {inv['period_start']} through {inv['period_end']} for Health evidence coverage.", "coverage", snapshot, {"topic": "temporal coverage", "period_start": inv["period_start"], "period_end": inv["period_end"], "period_count": inv["period_count"]}),
        source_package("srcpkg-campaign22-health-observed-missing", f"The Campaign 22 Health snapshot records {inv['observed_cells']} observed cells and {inv['explicit_missing_cells']} explicit missing cells.", "coverage", snapshot, {"topic": "observed missing coverage", "observed_cells": inv["observed_cells"], "explicit_missing_cells": inv["explicit_missing_cells"]}),
        source_package("srcpkg-campaign22-health-observed-share", f"The Campaign 22 Health snapshot has observed cell share {metrics['observed_share']} within the audited evidence grid.", "derived", snapshot, {"topic": "observed share", "observed_share": metrics["observed_share"]}),
        source_package("srcpkg-campaign22-health-missing-share", f"The Campaign 22 Health snapshot has explicit missing cell share {metrics['missing_share']} within the audited evidence grid.", "negative", snapshot, {"topic": "missing share", "missing_share": metrics["missing_share"]}),
        source_package("srcpkg-campaign22-health-family-count", f"The Campaign 22 Health inventory groups the selected indicators into {fam['family_count']} deterministic metadata families.", "classified", snapshot, {"topic": "family count", "family_count": fam["family_count"], "families": [r["family"] for r in fam["families"]]}),
        source_package("srcpkg-campaign22-health-partial-families", f"Campaign 22 records {metrics['partial_family_count']} Health metadata families with explicit missing cells in the audited evidence grid.", "negative", snapshot, {"topic": "partial family count", "partial_family_count": metrics["partial_family_count"]}),
        source_package("srcpkg-campaign22-health-provenance-state", "Campaign 22 records WDI Health source URLs, raw artifact URLs, raw artifact hashes, release keys, license notes, and WDI last-updated metadata as available in the evidence snapshot.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign22-health-evidence-quality-state", "Campaign 22 records explicit observation status, observed and missing status, WDI-only source scope, and annual-scalar evidence shape for the Health family.", "evidence_quality", snapshot, {"topic": "evidence quality state", **val}),
        source_package("srcpkg-campaign22-health-unsupported-dimensions", "The Campaign 22 Health annual-scalar evidence snapshot does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the accepted production scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": fam["unsupported_dimensions"]}),
        source_package("srcpkg-campaign22-replication-contract", "Campaign 22 uses unchanged production workflow, package construction, validator behaviour, Production Support, provenance handling, fingerprinting, reporting, metrics, and Production Evolution Log workflow from the mature WDI demographic production method.", "methodological", snapshot, {"topic": "replication contract", **repl}),
        source_package("srcpkg-campaign22-deterministic-transfer-method", "Campaign 22 derives Health evidence-quality and coverage objects deterministically from the immutable Health evidence snapshot.", "methodological", snapshot, {"topic": "deterministic transfer method", "input_snapshot": snapshot["snapshot_fingerprint"]}),
    ]
    for row in fam["families"]:
        packages.append(source_package(
            f"srcpkg-campaign22-family-{row['family'].replace('_', '-')}",
            f"Campaign 22 records {row['indicator_count']} annual-scalar indicators for the {row['family']} Health metadata family.",
            "coverage",
            snapshot,
            {"topic": "health metadata family", **row},
        ))
    return packages
...
def build_rejected_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    rejected = []
    bad_boundary = source_package("srcpkg-campaign22-reject-boundary-language", "This Health candidate includes a causal claim, a forecast, and a recommendation.", "coverage", snapshot, {"topic": "boundary rejection"})
    rejected.append(bad_boundary)
    missing_provenance = source_package("srcpkg-campaign22-reject-missing-provenance", "A malformed Campaign 22 candidate omits provenance fields.", "provenance", snapshot, {"topic": "malformed provenance"})
    missing_provenance.pop("provenance")
    rejected.append(missing_provenance)
    missing_fingerprint = source_package("srcpkg-campaign22-reject-missing-fingerprint", "A malformed Campaign 22 candidate omits lineage fingerprints.", "provenance", snapshot, {"topic": "malformed fingerprint"})
    missing_fingerprint.pop("fingerprints")
    rejected.append(missing_fingerprint)
    unsupported_category = source_package("srcpkg-campaign22-reject-unsupported-category", "A malformed Campaign 22 candidate uses an unsupported energy-mining-rating category.", "health_rating", snapshot, {"topic": "unsupported category"})
...
def repository_impact_assessment(accepted_objects: list[dict[str, Any]], health: dict[str, Any], previous_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Health source scope and WDI annual-scalar evidence-shape confirmation",
            "Health indicator inventory and metadata-family counts",
            "Health observed/missing evidence-grid totals and shares",
            "Health provenance-state and validation-state facts",
            "Health unsupported-dimension negative knowledge",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "opened WDI Health as the next Phase 2 production family",
        ...
        "repository_depth_gained": [
            "added family-entry evidence-quality/source-evidence baseline for Health maturation",
            "preserved source/provenance/validation evidence needed for later Health inventory and provenance-lineage closeout",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Health source-scope and indicator-count facts without rerunning the campaign snapshot",
            "downstream projects can reuse Health observed/missing evidence-grid totals and shares",
            "downstream projects can reuse Health unsupported-dimension and provenance-state facts",
            "future Health maturation campaigns can build on this family-entry baseline",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository health now tracks Health family-entry objects in indexes",
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    ...
    snapshot = immutable_wdi_health_snapshot()
    ...
    replay_snapshot = immutable_wdi_health_snapshot()
    ...
    comparison["campaign_22"] = {"accepted": len(accepted), "rejected": len(rejected), "determinism": determinism_verified, "fingerprint_stability": fingerprint_stability, "duplicates": duplicate_detected}
    ...
    transfer = {
        ...
        "campaign22_accepted": len(accepted),
        "campaign1_rejected": campaign1.get("rejected"),
        "campaign22_rejected": len(rejected),
        "observed_differences": [
            {"difference": "Health source family replaces demographic source family while preserving WDI annual-scalar shape.", "classification": "Health-specific"},
            {"difference": "Health accepted object count differs from Campaign 1 because Campaign 22 includes metadata-family coverage rows.", "classification": "evidence-family-specific"},
    ...
    quality.update({
        ...
        "processing_statistics": {
            ...
            "health_indicator_count": snapshot["audited_inventory"]["indicators_total"],
            "health_family_count": snapshot["health_family_inventory"]["family_count"],
            "observed_share": snapshot["coverage_metrics"]["observed_share"],
            "missing_share": snapshot["coverage_metrics"]["missing_share"],
        },
        "architectural_observations": [
            "Campaign 22 replicated the mature WDI demographic production methodology for WDI Health evidence without architecture change.",
            ...
            "Campaign 22 does not exercise multi-reference accepted objects; Health maturation remains the appropriate continuation.",
        ],
        "candidate_improvements_discovered": [
            "No validator, taxonomy, helper extraction, or architecture change is justified by Campaign 22.",
            "PEL-012 remains monitor because accepted Campaign 22 objects still use one evidence reference each.",
    ...
    quality["final_recommendation"] = "Continue maturing the WDI Health annual-scalar family under Operational Expansion; preserve the existing KnowledgeForge production methodology unchanged and populate the Knowledge Repository after validation."
    ...
    summary = {
        ...
        "final_recommendation": "Continue maturing the WDI Health annual-scalar family under Operational Expansion; preserve the existing KnowledgeForge production methodology unchanged and populate the Knowledge Repository after validation.",
...
def write_reports(output: Path, summary: dict[str, Any], quality: dict[str, Any], catalogue: list[dict[str, Any]], rejected: list[dict[str, Any]]) -> None:
    ...
    write_text(reports / "campaign_22_final_report.md", f"""
# Campaign 22 Final Report
...
Campaign 22 accepted {summary['accepted']} KnowledgeObjectPackages and preserved {summary['rejected']} rejected candidates.

The mature WDI demographic production methodology transferred to WDI Health evidence without architecture, taxonomy, validator, package-model, Production Support, provenance, fingerprint, reporting, or PEL workflow modification.
...
Campaign 22 matches Campaign 0, Campaign 1, and the mature demographic family on deterministic replay, fingerprint stability, provenance completeness, rejected-candidate preservation, and absence of observed duplicate pressure.
""")
    write_text(reports / "replication_assessment.md", f"""
# Replication Assessment

Campaign 22 transfers the established production methodology to WDI Health annual-scalar evidence after three Mature WDI families.
...
- Campaign 22 accepted: {transfer['campaign22_accepted']}
- Campaign 1 rejected: {transfer['campaign1_rejected']}
- Campaign 22 rejected: {transfer['campaign22_rejected']}

The accepted-object count differs because Campaign 22 includes Health metadata-family coverage rows. This difference is evidence-family-specific and does not imply architecture pressure.
""")
...
The mature WDI demographic production methodology transferred successfully to WDI Health annual-scalar evidence.
...
- Health-specific: caused by the selected Health family itself.
...
## What Campaign 22 showed

- The established WDI annual-scalar production methodology transferred to WDI Health after three Mature WDI families.
...
- Health evidence produced valid evidence-quality, coverage, factual, classified, derived, negative, provenance, and methodological objects.
...
Proceed to WDI Health maturation unchanged unless a Doctrine Review Trigger occurs.
""")
...
1. Current package and validator contracts handled WDI Health evidence-quality and coverage transfer without modification.
2. Existing taxonomy covered Health evidence-level production objects.
...
5. PEL-012 remains open because accepted Campaign 22 objects still use one evidence reference each.
6. No architecture redesign is supported.

## Unsupported changes

Campaign 22 does not support ontology/taxonomy change, validator modification, repository coupling, adapter/API/shared-schema work, database coupling, runtime infrastructure, local-model generation, frontier-model generation, or additional helper extraction.
""")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Campaign 22 WDI Health transfer production campaign")
