#!/usr/bin/env python3
"""Campaign 28: WDI Trade annual-scalar evidence-quality and source-evidence transfer.

Deterministic controlled production campaign. It uses an embedded immutable
WDI Trade annual-scalar evidence snapshot and the existing KnowledgeForge
...
CAMPAIGN_ID = "campaign-28-wdi-trade-annual-scalar-evidence-quality-source-evidence-transfer"
...
def immutable_wdi_trade_snapshot() -> dict[str, Any]:
    """Return the bounded immutable WDI Trade evidence snapshot.
    ...
    family_rows = [
        ...
        {"family": "trade_resources_metadata", "indicator_count": 20, "observed_series_cells": 98260, "missing_series_cells": 28740, "applicability": "annual scalar indicator metadata"},
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_trade",
        "scope": {
            ...
            "indicator_scope": "WDI Trade topic annual-scalar indicators selected by the Phase 2 Production Roadmap",
        },
        "audited_inventory": {
            "source": "World Bank World Development Indicators (WDI)",
            "wdi_topic_id": 21,
            "wdi_topic_name": "Trade",
            ...
            "dataset_releases": 3,
            "pipeline_runs": 8,
            "selection_source": "Campaign 28 planning gate and Phase 2 Production Roadmap",
        },
        "trade_family_inventory": {
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar trade evidence-quality and source-evidence transfer",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Trade evidence snapshot",
        ...
        classification={
            ...
            "evidence_kind": "audited_wdi_trade_snapshot_characteristic",
            "campaign_id": CAMPAIGN_ID,
        },
        validation_metadata={
        ...
        provenance={
            ...
            "selection_rule": "Phase 2 Production Roadmap selected WDI Trade annual-scalar evidence as the next production family after WDI Energy & Mining reached Mature",
            "source_family": snapshot["evidence_family"],
        },
        reproducibility={
            "state": "reproducible",
            "handle": f"python3 tools/run_campaign28_wdi_trade_transfer.py --output artifacts/production/{CAMPAIGN_ID}",
            "rerun_method": "deterministic embedded immutable WDI Trade evidence snapshot and canonical JSON construction",
            "nondeterminism": "none",
        },
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    fam = snapshot["trade_family_inventory"]
    ...
    packages = [
        source_package("srcpkg-campaign28-trade-source-scope", "The Campaign 28 source evidence package is scoped to World Bank WDI annual-scalar Trade evidence.", "factual", snapshot, {"topic": "source scope", "source": inv["source"], "frequency": snapshot["scope"]["frequency"], "shape": snapshot["scope"]["shape"]}),
        source_package("srcpkg-campaign28-trade-topic-inventory", f"The audited WDI Trade topic inventory records {inv['indicators_total']} annual-scalar indicators.", "coverage", snapshot, {"topic": "indicator coverage", "indicator_count": inv["indicators_total"], "wdi_topic_id": inv["wdi_topic_id"]}),
        source_package("srcpkg-campaign28-trade-territory-scope", f"Campaign 28 uses the same audited WDI territory scope count of {inv['territories_total']} non-aggregate or country-like territories.", "coverage", snapshot, {"topic": "territory coverage", "territory_count": inv["territories_total"]}),
        source_package("srcpkg-campaign28-trade-period-scope", f"Campaign 28 uses {inv['period_count']} annual periods from {inv['period_start']} through {inv['period_end']} for Trade evidence coverage.", "coverage", snapshot, {"topic": "temporal coverage", "period_start": inv["period_start"], "period_end": inv["period_end"], "period_count": inv["period_count"]}),
        source_package("srcpkg-campaign28-trade-observed-missing", f"The Campaign 28 Trade snapshot records {inv['observed_cells']} observed cells and {inv['explicit_missing_cells']} explicit missing cells.", "coverage", snapshot, {"topic": "observed missing coverage", "observed_cells": inv["observed_cells"], "explicit_missing_cells": inv["explicit_missing_cells"]}),
        source_package("srcpkg-campaign28-trade-observed-share", f"The Campaign 28 Trade snapshot has observed cell share {metrics['observed_share']} within the audited evidence grid.", "derived", snapshot, {"topic": "observed share", "observed_share": metrics["observed_share"]}),
        source_package("srcpkg-campaign28-trade-missing-share", f"The Campaign 28 Trade snapshot has explicit missing cell share {metrics['missing_share']} within the audited evidence grid.", "negative", snapshot, {"topic": "missing share", "missing_share": metrics["missing_share"]}),
        source_package("srcpkg-campaign28-trade-family-count", f"The Campaign 28 Trade inventory groups the selected indicators into {fam['family_count']} deterministic metadata families.", "classified", snapshot, {"topic": "family count", "family_count": fam["family_count"], "families": [r["family"] for r in fam["families"]]}),
        source_package("srcpkg-campaign28-trade-partial-families", f"Campaign 28 records {metrics['partial_family_count']} Trade metadata families with explicit missing cells in the audited evidence grid.", "negative", snapshot, {"topic": "partial family count", "partial_family_count": metrics["partial_family_count"]}),
        source_package("srcpkg-campaign28-trade-provenance-state", "Campaign 28 records WDI Trade source URLs, raw artifact URLs, raw artifact hashes, release keys, license notes, and WDI last-updated metadata as available in the evidence snapshot.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign28-trade-evidence-quality-state", "Campaign 28 records explicit observation status, observed and missing status, WDI-only source scope, and annual-scalar evidence shape for the Trade family.", "evidence_quality", snapshot, {"topic": "evidence quality state", **val}),
        source_package("srcpkg-campaign28-trade-unsupported-dimensions", "The Campaign 28 Trade annual-scalar evidence snapshot does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the accepted production scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": fam["unsupported_dimensions"]}),
        source_package("srcpkg-campaign28-replication-contract", "Campaign 28 uses unchanged production workflow, package construction, validator behaviour, Production Support, provenance handling, fingerprinting, reporting, metrics, and Production Evolution Log workflow from the mature WDI demographic production method.", "methodological", snapshot, {"topic": "replication contract", **repl}),
        source_package("srcpkg-campaign28-deterministic-transfer-method", "Campaign 28 derives Trade evidence-quality and coverage objects deterministically from the immutable Trade evidence snapshot.", "methodological", snapshot, {"topic": "deterministic transfer method", "input_snapshot": snapshot["snapshot_fingerprint"]}),
    ]
    for row in fam["families"]:
        packages.append(source_package(
            f"srcpkg-campaign28-family-{row['family'].replace('_', '-')}",
            f"Campaign 28 records {row['indicator_count']} annual-scalar indicators for the {row['family']} Trade metadata family.",
            "coverage",
            snapshot,
            {"topic": "trade metadata family", **row},
        ))
    return packages
...
def build_rejected_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    rejected = []
    bad_boundary = source_package("srcpkg-campaign28-reject-boundary-language", "This Trade candidate includes a causal claim, a forecast, and a recommendation.", "coverage", snapshot, {"topic": "boundary rejection"})
    rejected.append(bad_boundary)
    missing_provenance = source_package("srcpkg-campaign28-reject-missing-provenance", "A malformed Campaign 28 candidate omits provenance fields.", "provenance", snapshot, {"topic": "malformed provenance"})
    missing_provenance.pop("provenance")
    rejected.append(missing_provenance)
    missing_fingerprint = source_package("srcpkg-campaign28-reject-missing-fingerprint", "A malformed Campaign 28 candidate omits lineage fingerprints.", "provenance", snapshot, {"topic": "malformed fingerprint"})
    missing_fingerprint.pop("fingerprints")
    rejected.append(missing_fingerprint)
    unsupported_category = source_package("srcpkg-campaign28-reject-unsupported-category", "A malformed Campaign 28 candidate uses an unsupported energy-mining-rating category.", "trade_rating", snapshot, {"topic": "unsupported category"})
...
def repository_impact_assessment(accepted_objects: list[dict[str, Any]], trade: dict[str, Any], previous_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Trade source scope and WDI annual-scalar evidence-shape confirmation",
            "Trade indicator inventory and metadata-family counts",
            "Trade observed/missing evidence-grid totals and shares",
            "Trade provenance-state and validation-state facts",
            "Trade unsupported-dimension negative knowledge",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "opened WDI Trade as the next Phase 2 production family",
        ...
        "repository_depth_gained": [
            "added family-entry evidence-quality/source-evidence baseline for Trade maturation",
            "preserved source/provenance/validation evidence needed for later Trade inventory and provenance-lineage closeout",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Trade source-scope and indicator-count facts without rerunning the campaign snapshot",
            "downstream projects can reuse Trade observed/missing evidence-grid totals and shares",
            "downstream projects can reuse Trade unsupported-dimension and provenance-state facts",
            "future Trade maturation campaigns can build on this family-entry baseline",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository trade now tracks Trade family-entry objects in indexes",
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    ...
    snapshot = immutable_wdi_trade_snapshot()
    ...
    replay_snapshot = immutable_wdi_trade_snapshot()
    ...
    comparison["campaign_28"] = {"accepted": len(accepted), "rejected": len(rejected), "determinism": determinism_verified, "fingerprint_stability": fingerprint_stability, "duplicates": duplicate_detected}
    ...
    transfer = {
        ...
        "campaign28_accepted": len(accepted),
        "campaign1_rejected": campaign1.get("rejected"),
        "campaign28_rejected": len(rejected),
        "observed_differences": [
            {"difference": "Trade source family replaces demographic source family while preserving WDI annual-scalar shape.", "classification": "Trade-specific"},
            {"difference": "Trade accepted object count differs from Campaign 1 because Campaign 28 includes metadata-family coverage rows.", "classification": "evidence-family-specific"},
    ...
    quality.update({
        ...
        "processing_statistics": {
            ...
            "trade_indicator_count": snapshot["audited_inventory"]["indicators_total"],
            "trade_family_count": snapshot["trade_family_inventory"]["family_count"],
            "observed_share": snapshot["coverage_metrics"]["observed_share"],
            "missing_share": snapshot["coverage_metrics"]["missing_share"],
        },
        "architectural_observations": [
            "Campaign 28 replicated the mature WDI demographic production methodology for WDI Trade evidence without architecture change.",
            ...
            "Campaign 28 does not exercise multi-reference accepted objects; Trade maturation remains the appropriate continuation.",
        ],
        "candidate_improvements_discovered": [
            "No validator, taxonomy, helper extraction, or architecture change is justified by Campaign 28.",
            "PEL-012 remains monitor because accepted Campaign 28 objects still use one evidence reference each.",
    ...
    quality["repository_trade_summary"] = repository_trade_summary(Path(repository_root), knowledge_object_packages, repository_result, campaign_previous_count, quality["fingerprint_stability"])
    quality["knowledge_repository_impact_assessment"] = repository_impact_assessment(knowledge_object_packages, quality["repository_trade_summary"], campaign_previous_count)
    quality["final_recommendation"] = "Continue maturing the WDI Trade annual-scalar family under Operational Expansion; preserve the existing KnowledgeForge production methodology unchanged and populate the Knowledge Repository after validation."
    ...
    summary = {
        ...
        "final_recommendation": "Continue maturing the WDI Trade annual-scalar family under Operational Expansion; preserve the existing KnowledgeForge production methodology unchanged and populate the Knowledge Repository after validation.",
...
def write_reports(output: Path, summary: dict[str, Any], quality: dict[str, Any], catalogue: list[dict[str, Any]], rejected: list[dict[str, Any]]) -> None:
    ...
    write_text(reports / "campaign_28_final_report.md", f"""
# Campaign 28 Final Report
...
Campaign 28 accepted {summary['accepted']} KnowledgeObjectPackages and preserved {summary['rejected']} rejected candidates.

The mature WDI demographic production methodology transferred to WDI Trade evidence without architecture, taxonomy, validator, package-model, Production Support, provenance, fingerprint, reporting, or PEL workflow modification.
...
Campaign 28 matches Campaign 0, Campaign 1, and the mature demographic family on deterministic replay, fingerprint stability, provenance completeness, rejected-candidate preservation, and absence of observed duplicate pressure.
""")
    write_text(reports / "replication_assessment.md", f"""
# Replication Assessment

Campaign 28 transfers the established production methodology to WDI Trade annual-scalar evidence after three Mature WDI families.
...
- Campaign 28 accepted: {transfer['campaign28_accepted']}
- Campaign 1 rejected: {transfer['campaign1_rejected']}
- Campaign 28 rejected: {transfer['campaign28_rejected']}

The accepted-object count differs because Campaign 28 includes Trade metadata-family coverage rows. This difference is evidence-family-specific and does not imply architecture pressure.
""")
...
The mature WDI demographic production methodology transferred successfully to WDI Trade annual-scalar evidence.
...
- Trade-specific: caused by the selected Trade family itself.
...
## What Campaign 28 showed

- The established WDI annual-scalar production methodology transferred to WDI Trade after three Mature WDI families.
...
- Trade evidence produced valid evidence-quality, coverage, factual, classified, derived, negative, provenance, and methodological objects.
...
Proceed to WDI Trade maturation unchanged unless a Doctrine Review Trigger occurs.
""")
...
1. Current package and validator contracts handled WDI Trade evidence-quality and coverage transfer without modification.
2. Existing taxonomy covered Trade evidence-level production objects.
...
5. PEL-012 remains open because accepted Campaign 28 objects still use one evidence reference each.
6. No architecture redesign is supported.

## Unsupported changes

Campaign 28 does not support ontology/taxonomy change, validator modification, repository coupling, adapter/API/shared-schema work, database coupling, runtime infrastructure, local-model generation, frontier-model generation, or additional helper extraction.
""")
    trade = quality["repository_trade_summary"]
    write_text(reports / "repository_trade_summary.md", f"""
# Repository Trade Summary
...
{markdown_table([[k, v] for k, v in trade['objects_by_evidence_family'].items()], ['Evidence family', 'Count'])}

## Objects by knowledge category

{markdown_table([[k, v] for k, v in trade['objects_by_knowledge_category'].items()], ['Knowledge category', 'Count'])}

## Objects by lifecycle state

{markdown_table([[k, v] for k, v in trade['objects_by_lifecycle_state'].items()], ['Lifecycle state', 'Count'])}
""")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Campaign 28 WDI Trade transfer production campaign")
