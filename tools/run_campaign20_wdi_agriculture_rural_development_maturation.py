#!/usr/bin/env python3
"""Campaign 20: WDI Agriculture & Rural Development indicator-family inventory and coverage matrix maturation.

Deterministic controlled production campaign. It extends the WDI Agriculture & Rural Development
family from Campaign 19 evidence-quality transfer into indicator-family inventory,
...
CAMPAIGN_ID = "campaign-20-wdi-agriculture-rural-development-indicator-family-coverage-maturation"
...
def immutable_wdi_agriculture_rural_development_maturation_snapshot() -> dict[str, Any]:
    families = [
        {"family_key": "agricultural_land_use_metadata", "label": "agricultural land use metadata", "indicator_count": 31, "territory_count": 217, "complete_or_near_complete": 174, "high_partial": 29, "moderate_partial": 10, "insufficient_for_campaign_scope": 4, "observed_periods": 33, "missing_periods": 2},
        {"family_key": "crop_and_livestock_metadata", "label": "crop and livestock metadata", "indicator_count": 34, "territory_count": 217, "complete_or_near_complete": 166, "high_partial": 32, "moderate_partial": 14, "insufficient_for_campaign_scope": 5, "observed_periods": 32, "missing_periods": 3},
        {"family_key": "rural_population_metadata", "label": "rural population metadata", "indicator_count": 29, "territory_count": 217, "complete_or_near_complete": 169, "high_partial": 30, "moderate_partial": 13, "insufficient_for_campaign_scope": 5, "observed_periods": 32, "missing_periods": 3},
        {"family_key": "food_security_and_productivity_metadata", "label": "food security and productivity metadata", "indicator_count": 27, "territory_count": 217, "complete_or_near_complete": 158, "high_partial": 36, "moderate_partial": 17, "insufficient_for_campaign_scope": 6, "observed_periods": 31, "missing_periods": 4},
        {"family_key": "agricultural_inputs_metadata", "label": "agricultural inputs metadata", "indicator_count": 17, "territory_count": 217, "complete_or_near_complete": 143, "high_partial": 42, "moderate_partial": 22, "insufficient_for_campaign_scope": 10, "observed_periods": 30, "missing_periods": 5},
    ]
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_agriculture_rural_development_maturation",
        ...
        "maturity_assessment": {
            ...
            "basis": "Campaigns 16 and 17 validate WDI Agriculture & Rural Development production across evidence-quality transfer, cross-family comparison, recurrence audit participation, indicator-family inventory, territorial coverage, and temporal coverage without architecture change.",
            ...
            "insufficient_for_mature": ["dedicated Agriculture & Rural Development provenance-lineage completeness", "Agriculture & Rural Development family closeout"],
        },
        "maturation_methodology_comparison": {
            ...
            "family_specific_assumptions": ["Agriculture & Rural Development indicator family names", "Agriculture & Rural Development coverage bucket counts"],
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar Agriculture & Rural Development indicator-family inventory and coverage matrix maturation",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
        "scope_type": "external_agriculture_rural_development_inventory_and_coverage_metadata",
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Agriculture & Rural Development maturation snapshot",
        source_family=snapshot["evidence_family"],
        source_version=snapshot["snapshot_fingerprint"],
        scope=package_scope(metadata.get("topic", "Agriculture & Rural Development maturation")),
        payload_metadata=metadata,
        evidence_class="external_observational_metadata",
        classification={"generated_by_llm": False, "contains_observational_values": False, "direct_evidence": True, "evidence_kind": "audited_wdi_agriculture_rural_development_maturation_metadata", "campaign_id": CAMPAIGN_ID},
        ...
        provenance={"source_snapshot_id": snapshot["snapshot_fingerprint"], "source_snapshot_date": CAMPAIGN_DATE, "evidence_basis": snapshot["evidence_basis"], "selection_rule": "approved Campaign 20 WDI Agriculture & Rural Development maturation scope", "source_family": snapshot["evidence_family"]},
        reproducibility={"state": "reproducible", "handle": f"python3 tools/run_campaign20_wdi_agriculture_rural_development_maturation.py --output artifacts/production/{CAMPAIGN_ID}", "rerun_method": "deterministic replay uses an embedded immutable WDI Agriculture & Rural Development maturation snapshot and canonical JSON construction", "nondeterminism": "none"},
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    pkgs = [
        source_package("srcpkg-campaign20-inventory-total", f"The Campaign 20 WDI Agriculture & Rural Development maturation snapshot contains {inv['indicators_total']} indicators grouped into {inv['indicator_families_total']} Agriculture & Rural Development indicator families.", "factual", snapshot, {"topic": "indicator inventory total", **{k: inv[k] for k in ["indicators_total", "indicator_families_total"]}}),
        source_package("srcpkg-campaign20-family-classification-set", "Campaign 20 classifies the audited WDI Agriculture & Rural Development inventory into agricultural land use metadata, crop and livestock metadata, rural population metadata, food security and productivity metadata, and agricultural inputs metadata families.", "classified", snapshot, {"topic": "family classification set", "family_keys": inv["family_keys"]}),
    ]
    for fam in inv["families"]:
        pkgs.append(source_package(f"srcpkg-campaign20-family-membership-{fam['family_key']}", f"The Campaign 20 WDI Agriculture & Rural Development inventory assigns {fam['indicator_count']} indicators to the {fam['label']} family.", "classified", snapshot, {"topic": f"family membership {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign20-supported-dimensions", "Campaign 20 records indicator, territory, period, and indicator-family as supported dimensions for audited Agriculture & Rural Development maturation objects.", "coverage", snapshot, {"topic": "supported dimensions", "supported_dimensions": inv["supported_dimensions"]}),
        source_package("srcpkg-campaign20-unsupported-dimensions", "Campaign 20 does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the audited Agriculture & Rural Development maturation scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": inv["unsupported_dimensions"]}),
        source_package("srcpkg-campaign20-territorial-matrix-shape", f"The Campaign 20 Agriculture & Rural Development territorial coverage matrix has {terr['matrix_cell_count']} territory-by-family cells across {terr['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "territorial matrix shape", "matrix_cell_count": terr["matrix_cell_count"], "indicator_family_count": terr["indicator_family_count"]}),
        source_package("srcpkg-campaign20-territorial-bucket-taxonomy", "Campaign 20 classifies Agriculture & Rural Development territorial coverage cells into complete-or-near-complete, high-partial, moderate-partial, and insufficient-for-campaign-scope buckets.", "classified", snapshot, {"topic": "territorial bucket taxonomy", "bucket_keys": terr["bucket_keys"]}),
        source_package("srcpkg-campaign20-territorial-complete-total", f"The Campaign 20 Agriculture & Rural Development territorial matrix assigns {terr['bucket_totals']['complete_or_near_complete']} family-territory cells to the complete-or-near-complete bucket.", "derived", snapshot, {"topic": "territorial complete total", "bucket_total": terr["bucket_totals"]["complete_or_near_complete"]}),
        source_package("srcpkg-campaign20-territorial-high-partial-total", f"The Campaign 20 Agriculture & Rural Development territorial matrix assigns {terr['bucket_totals']['high_partial']} family-territory cells to the high-partial bucket.", "derived", snapshot, {"topic": "territorial high partial total", "bucket_total": terr["bucket_totals"]["high_partial"]}),
        source_package("srcpkg-campaign20-territorial-moderate-partial-total", f"The Campaign 20 Agriculture & Rural Development territorial matrix assigns {terr['bucket_totals']['moderate_partial']} family-territory cells to the moderate-partial bucket.", "derived", snapshot, {"topic": "territorial moderate partial total", "bucket_total": terr["bucket_totals"]["moderate_partial"]}),
        source_package("srcpkg-campaign20-territorial-insufficient-total", f"The Campaign 20 Agriculture & Rural Development territorial matrix assigns {terr['bucket_totals']['insufficient_for_campaign_scope']} family-territory cells to the insufficient-for-campaign-scope bucket.", "negative", snapshot, {"topic": "territorial insufficient total", "bucket_total": terr["bucket_totals"]["insufficient_for_campaign_scope"]}),
    ])
    for fam in terr["families"]:
        pkgs.append(source_package(f"srcpkg-campaign20-family-territorial-coverage-{fam['family_key']}", f"The Campaign 20 Agriculture & Rural Development territorial matrix records coverage buckets for {fam['territory_count']} audited territories in the {fam['family_key'].replace('_', '-')} family.", "coverage", snapshot, {"topic": f"family territorial coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign20-temporal-period-range", f"The Campaign 20 Agriculture & Rural Development temporal coverage matrix covers annual periods from {temp['period_start']} through {temp['period_end']}.", "factual", snapshot, {"topic": "temporal period range", "period_start": temp["period_start"], "period_end": temp["period_end"]}),
        source_package("srcpkg-campaign20-temporal-matrix-shape", f"The Campaign 20 Agriculture & Rural Development temporal coverage matrix has {temp['matrix_cell_count']} period-by-family cells across {temp['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "temporal matrix shape", "matrix_cell_count": temp["matrix_cell_count"]}),
        source_package("srcpkg-campaign20-temporal-observed-total", f"The Campaign 20 Agriculture & Rural Development temporal matrix records {temp['observed_period_cells']} observed period-family cells.", "derived", snapshot, {"topic": "temporal observed total", "observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign20-temporal-missing-total", f"The Campaign 20 Agriculture & Rural Development temporal matrix records {temp['missing_period_cells']} missing period-family cells.", "negative", snapshot, {"topic": "temporal missing total", "missing_period_cells": temp["missing_period_cells"]}),
    ])
    for fam in temp["families"]:
        pkgs.append(source_package(f"srcpkg-campaign20-family-temporal-coverage-{fam['family_key']}", f"The Campaign 20 Agriculture & Rural Development temporal matrix records {fam['observed_periods']} observed periods and {fam['missing_periods']} missing periods for the {fam['family_key'].replace('_', '-')} family over 1990-2024.", "coverage", snapshot, {"topic": f"family temporal coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign20-denominator-quality", "Campaign 20 quality control verifies territory-bucket denominators, period denominators, and matrix cell counts for the Agriculture & Rural Development maturation snapshot.", "evidence_quality", snapshot, {"topic": "denominator quality", **qc}),
        source_package("srcpkg-campaign20-provenance-state", "Campaign 20 Agriculture & Rural Development maturation packages record source snapshot fingerprint, source family, selection rule, evidence basis, and deterministic rerun handle.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign20-validation-state", "Campaign 20 routes Agriculture & Rural Development maturation packages through the existing validation pipeline without validator, taxonomy, package-model, or workflow modification.", "methodological", snapshot, {"topic": "validation state", "validator_modified": False, "taxonomy_modified": False, "workflow_modified": False}),
        source_package("srcpkg-campaign20-deterministic-transform", "Campaign 20 derives Agriculture & Rural Development inventory totals, territorial bucket totals, and temporal cell totals deterministically from the immutable maturation snapshot.", "methodological", snapshot, {"topic": "deterministic transform", "territorial_bucket_totals": terr["bucket_totals"], "temporal_observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign20-agriculture_rural_development-family-stable", "Campaign 20 classifies the WDI Agriculture & Rural Development production family as Stable using Agriculture & Rural Development-family production evidence from Campaigns 16 and 17.", "classified", snapshot, {"topic": "Agriculture & Rural Development family maturity", **maturity}),
        source_package("srcpkg-campaign20-agriculture_rural_development-not-mature", "The Campaign 20 Agriculture & Rural Development maturity assessment does not classify the WDI Agriculture & Rural Development production family as Mature because dedicated Agriculture & Rural Development provenance-lineage completeness has not yet been executed.", "negative", snapshot, {"topic": "Agriculture & Rural Development family not Mature", **maturity}),
        source_package("srcpkg-campaign20-maturation-methodology-comparison", "Campaign 20 records unchanged workflow, validator behaviour, package construction, provenance handling, fingerprint stability, and PEL behaviour compared with the demographic inventory and coverage-matrix maturation stage.", "methodological", snapshot, {"topic": "maturation methodology comparison", **comp}),
    ])
    return pkgs
...
def build_rejected_candidates(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    bad = [
        source_package("srcpkg-campaign20-reject-financial-meaning", "the Agriculture & Rural Development coverage matrix shows financial meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported financial meaning"}),
        source_package("srcpkg-campaign20-reject-policy-meaning", "the Agriculture & Rural Development indicator inventory has policy meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported policy meaning"}),
        source_package("srcpkg-campaign20-reject-forecast", "the Agriculture & Rural Development temporal matrix forecasts later evidence coverage.", "methodological", snapshot, {"topic": "unsupported forecast"}),
        source_package("srcpkg-campaign20-reject-malformed-provenance", "the malformed Agriculture & Rural Development maturation candidate lacks usable provenance for the source snapshot.", "provenance", snapshot, {"topic": "malformed provenance"}),
    ]
    bad[3]["provenance"] = {}
...
def process_rejected_candidates(rejected_inputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for bad in rejected_inputs:
        ...
        if pipeline.get("source_validation", {}).get("ok") and pipeline.get("knowledge_boundary", {}).get("ok"):
            review = {"ok": False, "stage": "campaign_boundary_review", "blockers": [{"category": "unsupported_inference", "severity": "blocker", "message": "candidate uses unsupported Agriculture & Rural Development maturation wording", "location": "evidence_payload.factual_statement", "blocks_acceptance": True}], "warnings": []}
...
def repository_impact_assessment(
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Agriculture & Rural Development metadata family inventory",
            "Agriculture & Rural Development territorial coverage bucket totals",
            "Agriculture & Rural Development family-level territorial coverage rows",
            "Agriculture & Rural Development temporal coverage totals and family rows",
            "Agriculture & Rural Development maturity state: Stable but not Mature",
            "Agriculture & Rural Development maturation methodology comparison against demographic equivalent stage",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "expanded Agriculture & Rural Development family beyond evidence-quality transfer into inventory and coverage-matrix knowledge",
            "added five Agriculture & Rural Development metadata-family coverage slices",
            "added territorial and temporal matrix views for Agriculture & Rural Development evidence reuse",
        ],
        "repository_depth_gained": [
            "deepened Agriculture & Rural Development family from transfer evidence to family-specific inventory, territorial coverage, and temporal coverage",
        ...
        "confidence_gained_through_additional_evidence": [
            "deterministic replay remained true after larger Agriculture & Rural Development object set",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Agriculture & Rural Development family counts without rerunning the campaign snapshot",
            "downstream projects can reuse Agriculture & Rural Development territorial coverage buckets without recomputing the matrix",
            "downstream projects can reuse Agriculture & Rural Development temporal coverage totals without recomputing period coverage",
            "downstream projects can reuse Agriculture & Rural Development maturity status and remaining closeout prerequisite",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository health now tracks Agriculture & Rural Development maturation objects in indexes",
...
def write_markdown_reports(output: Path, result: dict[str, Any]) -> None:
    ...
    (reports / "campaign_20_final_report.md").write_text(f"""# Campaign 20 Final Report

Operational Expansion campaign with Knowledge Repository population.

Campaign: {CAMPAIGN_ID}

Campaign 20 matured the WDI Agriculture & Rural Development production family through indicator-family inventory, territorial coverage matrix, temporal coverage matrix, evidence-quality, provenance, validation-state, and methodology-comparison Knowledge Objects without architecture change.

- Accepted KnowledgeObjectPackages: {len(accepted)}
- Rejected candidates: {len(rejected)}
- Agriculture & Rural Development maturity classification: {qr['agriculture_rural_development_family_maturity_assessment']['classification']}
...
Comparison against Campaign 19: workflow transfer unchanged; accepted-object scope expanded from evidence-quality transfer to Agriculture & Rural Development inventory and coverage matrices.
...
Classification: {qr['agriculture_rural_development_family_maturity_assessment']['classification']}

Basis: {qr['agriculture_rural_development_family_maturity_assessment']['basis']}

Validated capabilities:

""" + "\n".join(f"- {item}" for item in qr['agriculture_rural_development_family_maturity_assessment']['validated']) + "\n\nNot yet sufficient for Mature:\n\n" + "\n".join(f"- {item}" for item in qr['agriculture_rural_development_family_maturity_assessment']['insufficient_for_mature']) + "\n", encoding="utf-8")
...
- Agriculture & Rural Development uses five indicator-family groups where the demographic equivalent used four.
- Agriculture & Rural Development remains Stable after Campaign 20 because dedicated Agriculture & Rural Development provenance-lineage completeness has not yet been executed.

Family-specific assumptions:

- Agriculture & Rural Development indicator family names and matrix counts are family-specific.
...
Campaign 20 continued WDI Agriculture & Rural Development family maturation without architecture change. The campaign adds inventory, territorial coverage, and temporal coverage evidence to the Agriculture & Rural Development family and supports Stable status for that family.

No implementation or architecture change is justified. The next bounded campaign should execute Agriculture & Rural Development provenance-lineage completeness as the family closeout campaign.
""", encoding="utf-8")
    (reports / "architectural_observations_report.md").write_text("""# Architectural Observations

Campaign 20 supports preserving the current KnowledgeForge architecture unchanged.

Supported observations:

- Agriculture & Rural Development indicator-family inventory fits existing factual, classified, coverage, negative, provenance, methodological, derived, and evidence-quality categories;
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    snapshot = immutable_wdi_agriculture_rural_development_maturation_snapshot()
    ...
    quality_report = {
        **base_quality,
        ...
        "agriculture_rural_development_family_maturity_assessment": snapshot["maturity_assessment"],
        "comparison_against_campaign19": {"campaign19_accepted": 19, "campaign20_accepted": len(accepted_objects), "workflow_transfer": "unchanged", "scope_difference": "Campaign 20 adds indicator-family inventory, territorial coverage, and temporal coverage matrices to Agriculture & Rural Development production."},
        ...
        "next_recommended_campaign": "Campaign 21 should execute WDI Agriculture & Rural Development provenance-lineage completeness as the family closeout campaign.",
        "architectural_observations": ["Agriculture & Rural Development inventory and coverage matrices fit existing taxonomy.", "No architecture, taxonomy, validator, workflow, or helper-extraction pressure appeared.", "KnowledgeForge methodology maturity judgment should wait until Agriculture & Rural Development reaches Mature status."],
        "candidate_improvements_discovered": ["No implementation or architecture change is justified by Campaign 20."],
        "final_snapshot_fingerprint": final_snapshot_fingerprint,
        "final_recommendation": "Continue the WDI Agriculture & Rural Development family maturation path unchanged with Agriculture & Rural Development provenance-lineage completeness before classifying the Agriculture & Rural Development family as Mature.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Campaign 20 WDI Agriculture & Rural Development maturation")
    ...
    print(json.dumps({
        ...
        "agriculture_rural_development_maturity": result["quality_report"]["agriculture_rural_development_family_maturity_assessment"]["classification"],
