#!/usr/bin/env python3
"""Campaign 26: WDI Education indicator-family inventory and coverage matrix maturation.

Deterministic controlled production campaign. It extends the WDI Education
family from Campaign 25 evidence-quality transfer into indicator-family inventory,
...
CAMPAIGN_ID = "campaign-26-wdi-education-indicator-family-coverage-maturation"
...
def immutable_wdi_education_maturation_snapshot() -> dict[str, Any]:
    families = [
        ...
        {"family_key": "education_resources_metadata", "label": "education resources metadata", "indicator_count": 24, "territory_count": 217, "complete_or_near_complete": 154, "high_partial": 38, "moderate_partial": 18, "insufficient_for_campaign_scope": 7, "observed_periods": 31, "missing_periods": 4},
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_education_maturation",
        ...
        "maturity_assessment": {
            ...
            "basis": "Campaigns 16 and 17 validate WDI Education production across evidence-quality transfer, cross-family comparison, recurrence audit participation, indicator-family inventory, territorial coverage, and temporal coverage without architecture change.",
            ...
            "insufficient_for_mature": ["dedicated Education provenance-lineage completeness", "Education family closeout"],
        },
        "maturation_methodology_comparison": {
            ...
            "family_specific_assumptions": ["Education indicator family names", "Education coverage bucket counts"],
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar Education indicator-family inventory and coverage matrix maturation",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
        "scope_type": "external_education_inventory_and_coverage_metadata",
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Education maturation snapshot",
        source_family=snapshot["evidence_family"],
        source_version=snapshot["snapshot_fingerprint"],
        scope=package_scope(metadata.get("topic", "Education maturation")),
        payload_metadata=metadata,
        evidence_class="external_observational_metadata",
        classification={"generated_by_llm": False, "contains_observational_values": False, "direct_evidence": True, "evidence_kind": "audited_wdi_education_maturation_metadata", "campaign_id": CAMPAIGN_ID},
        ...
        provenance={"source_snapshot_id": snapshot["snapshot_fingerprint"], "source_snapshot_date": CAMPAIGN_DATE, "evidence_basis": snapshot["evidence_basis"], "selection_rule": "approved Campaign 26 WDI Education maturation scope", "source_family": snapshot["evidence_family"]},
        reproducibility={"state": "reproducible", "handle": f"python3 tools/run_campaign26_wdi_education_maturation.py --output artifacts/production/{CAMPAIGN_ID}", "rerun_method": "deterministic replay uses an embedded immutable WDI Education maturation snapshot and canonical JSON construction", "nondeterminism": "none"},
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    pkgs = [
        source_package("srcpkg-campaign26-inventory-total", f"The Campaign 26 WDI Education maturation snapshot contains {inv['indicators_total']} indicators grouped into {inv['indicator_families_total']} Education indicator families.", "factual", snapshot, {"topic": "indicator inventory total", **{k: inv[k] for k in ["indicators_total", "indicator_families_total"]}}),
        source_package("srcpkg-campaign26-family-classification-set", "Campaign 26 classifies the audited WDI Education inventory into agricultural land use metadata, crop and livestock metadata, rural population metadata, food security and productivity metadata, and agricultural inputs metadata families.", "classified", snapshot, {"topic": "family classification set", "family_keys": inv["family_keys"]}),
    ]
    for fam in inv["families"]:
        pkgs.append(source_package(f"srcpkg-campaign26-family-membership-{fam['family_key']}", f"The Campaign 26 WDI Education inventory assigns {fam['indicator_count']} indicators to the {fam['label']} family.", "classified", snapshot, {"topic": f"family membership {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign26-supported-dimensions", "Campaign 26 records indicator, territory, period, and indicator-family as supported dimensions for audited Education maturation objects.", "coverage", snapshot, {"topic": "supported dimensions", "supported_dimensions": inv["supported_dimensions"]}),
        source_package("srcpkg-campaign26-unsupported-dimensions", "Campaign 26 does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the audited Education maturation scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": inv["unsupported_dimensions"]}),
        source_package("srcpkg-campaign26-territorial-matrix-shape", f"The Campaign 26 Education territorial coverage matrix has {terr['matrix_cell_count']} territory-by-family cells across {terr['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "territorial matrix shape", "matrix_cell_count": terr["matrix_cell_count"], "indicator_family_count": terr["indicator_family_count"]}),
        source_package("srcpkg-campaign26-territorial-bucket-taxonomy", "Campaign 26 classifies Education territorial coverage cells into complete-or-near-complete, high-partial, moderate-partial, and insufficient-for-campaign-scope buckets.", "classified", snapshot, {"topic": "territorial bucket taxonomy", "bucket_keys": terr["bucket_keys"]}),
        source_package("srcpkg-campaign26-territorial-complete-total", f"The Campaign 26 Education territorial matrix assigns {terr['bucket_totals']['complete_or_near_complete']} family-territory cells to the complete-or-near-complete bucket.", "derived", snapshot, {"topic": "territorial complete total", "bucket_total": terr["bucket_totals"]["complete_or_near_complete"]}),
        source_package("srcpkg-campaign26-territorial-high-partial-total", f"The Campaign 26 Education territorial matrix assigns {terr['bucket_totals']['high_partial']} family-territory cells to the high-partial bucket.", "derived", snapshot, {"topic": "territorial high partial total", "bucket_total": terr["bucket_totals"]["high_partial"]}),
        source_package("srcpkg-campaign26-territorial-moderate-partial-total", f"The Campaign 26 Education territorial matrix assigns {terr['bucket_totals']['moderate_partial']} family-territory cells to the moderate-partial bucket.", "derived", snapshot, {"topic": "territorial moderate partial total", "bucket_total": terr["bucket_totals"]["moderate_partial"]}),
        source_package("srcpkg-campaign26-territorial-insufficient-total", f"The Campaign 26 Education territorial matrix assigns {terr['bucket_totals']['insufficient_for_campaign_scope']} family-territory cells to the insufficient-for-campaign-scope bucket.", "negative", snapshot, {"topic": "territorial insufficient total", "bucket_total": terr["bucket_totals"]["insufficient_for_campaign_scope"]}),
    ])
    for fam in terr["families"]:
        pkgs.append(source_package(f"srcpkg-campaign26-family-territorial-coverage-{fam['family_key']}", f"The Campaign 26 Education territorial matrix records coverage buckets for {fam['territory_count']} audited territories in the {fam['family_key'].replace('_', '-')} family.", "coverage", snapshot, {"topic": f"family territorial coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign26-temporal-period-range", f"The Campaign 26 Education temporal coverage matrix covers annual periods from {temp['period_start']} through {temp['period_end']}.", "factual", snapshot, {"topic": "temporal period range", "period_start": temp["period_start"], "period_end": temp["period_end"]}),
        source_package("srcpkg-campaign26-temporal-matrix-shape", f"The Campaign 26 Education temporal coverage matrix has {temp['matrix_cell_count']} period-by-family cells across {temp['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "temporal matrix shape", "matrix_cell_count": temp["matrix_cell_count"]}),
        source_package("srcpkg-campaign26-temporal-observed-total", f"The Campaign 26 Education temporal matrix records {temp['observed_period_cells']} observed period-family cells.", "derived", snapshot, {"topic": "temporal observed total", "observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign26-temporal-missing-total", f"The Campaign 26 Education temporal matrix records {temp['missing_period_cells']} missing period-family cells.", "negative", snapshot, {"topic": "temporal missing total", "missing_period_cells": temp["missing_period_cells"]}),
    ])
    for fam in temp["families"]:
        pkgs.append(source_package(f"srcpkg-campaign26-family-temporal-coverage-{fam['family_key']}", f"The Campaign 26 Education temporal matrix records {fam['observed_periods']} observed periods and {fam['missing_periods']} missing periods for the {fam['family_key'].replace('_', '-')} family over 1990-2024.", "coverage", snapshot, {"topic": f"family temporal coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign26-denominator-quality", "Campaign 26 quality control verifies territory-bucket denominators, period denominators, and matrix cell counts for the Education maturation snapshot.", "evidence_quality", snapshot, {"topic": "denominator quality", **qc}),
        source_package("srcpkg-campaign26-provenance-state", "Campaign 26 Education maturation packages record source snapshot fingerprint, source family, selection rule, evidence basis, and deterministic rerun handle.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign26-validation-state", "Campaign 26 routes Education maturation packages through the existing validation pipeline without validator, taxonomy, package-model, or workflow modification.", "methodological", snapshot, {"topic": "validation state", "validator_modified": False, "taxonomy_modified": False, "workflow_modified": False}),
        source_package("srcpkg-campaign26-deterministic-transform", "Campaign 26 derives Education inventory totals, territorial bucket totals, and temporal cell totals deterministically from the immutable maturation snapshot.", "methodological", snapshot, {"topic": "deterministic transform", "territorial_bucket_totals": terr["bucket_totals"], "temporal_observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign26-education-family-stable", "Campaign 26 classifies the WDI Education production family as Stable using Education-family production evidence from Campaigns 16 and 17.", "classified", snapshot, {"topic": "Education family maturity", **maturity}),
        source_package("srcpkg-campaign26-education-not-mature", "The Campaign 26 Education maturity assessment does not classify the WDI Education production family as Mature because dedicated Education provenance-lineage completeness has not yet been executed.", "negative", snapshot, {"topic": "Education family not Mature", **maturity}),
        source_package("srcpkg-campaign26-maturation-methodology-comparison", "Campaign 26 records unchanged workflow, validator behaviour, package construction, provenance handling, fingerprint stability, and PEL behaviour compared with the demographic inventory and coverage-matrix maturation stage.", "methodological", snapshot, {"topic": "maturation methodology comparison", **comp}),
    ])
    return pkgs
...
def build_rejected_candidates(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    bad = [
        source_package("srcpkg-campaign26-reject-financial-meaning", "the Education coverage matrix shows financial meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported financial meaning"}),
        source_package("srcpkg-campaign26-reject-policy-meaning", "the Education indicator inventory has policy meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported policy meaning"}),
        source_package("srcpkg-campaign26-reject-forecast", "the Education temporal matrix forecasts later evidence coverage.", "methodological", snapshot, {"topic": "unsupported forecast"}),
        source_package("srcpkg-campaign26-reject-malformed-provenance", "the malformed Education maturation candidate lacks usable provenance for the source snapshot.", "provenance", snapshot, {"topic": "malformed provenance"}),
    ]
    bad[3]["provenance"] = {}
...
def process_rejected_candidates(rejected_inputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for bad in rejected_inputs:
        ...
        if pipeline.get("source_validation", {}).get("ok") and pipeline.get("knowledge_boundary", {}).get("ok"):
            review = {"ok": False, "stage": "campaign_boundary_review", "blockers": [{"category": "unsupported_inference", "severity": "blocker", "message": "candidate uses unsupported Education maturation wording", "location": "evidence_payload.factual_statement", "blocks_acceptance": True}], "warnings": []}
...
def repository_impact_assessment(
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Education metadata family inventory",
            "Education territorial coverage bucket totals",
            "Education family-level territorial coverage rows",
            "Education temporal coverage totals and family rows",
            "Education maturity state: Stable but not Mature",
            "Education maturation methodology comparison against demographic equivalent stage",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "expanded Education family beyond evidence-quality transfer into inventory and coverage-matrix knowledge",
            "added five Education metadata-family coverage slices",
            "added territorial and temporal matrix views for Education evidence reuse",
        ],
        "repository_depth_gained": [
            "deepened Education family from transfer evidence to family-specific inventory, territorial coverage, and temporal coverage",
        ...
        "confidence_gained_through_additional_evidence": [
            "deterministic replay remained true after larger Education object set",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Education family counts without rerunning the campaign snapshot",
            "downstream projects can reuse Education territorial coverage buckets without recomputing the matrix",
            "downstream projects can reuse Education temporal coverage totals without recomputing period coverage",
            "downstream projects can reuse Education maturity status and remaining closeout prerequisite",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository education now tracks Education maturation objects in indexes",
...
def write_markdown_reports(output: Path, result: dict[str, Any]) -> None:
    ...
    (reports / "campaign_26_final_report.md").write_text(f"""# Campaign 26 Final Report

Operational Expansion campaign with Knowledge Repository population.

Campaign: {CAMPAIGN_ID}

Campaign 26 matured the WDI Education production family through indicator-family inventory, territorial coverage matrix, temporal coverage matrix, evidence-quality, provenance, validation-state, and methodology-comparison Knowledge Objects without architecture change.

- Accepted KnowledgeObjectPackages: {len(accepted)}
- Rejected candidates: {len(rejected)}
- Education maturity classification: {qr['education_family_maturity_assessment']['classification']}
...
- Knowledge Repository fingerprint: `{qr['repository_education_summary']['repository_fingerprint']}`
...
Comparison against Campaign 25: workflow transfer unchanged; accepted-object scope expanded from evidence-quality transfer to Education inventory and coverage matrices.
...
""", encoding="utf-8")
    education = qr["repository_education_summary"]
...
Classification: {qr['education_family_maturity_assessment']['classification']}

Basis: {qr['education_family_maturity_assessment']['basis']}

Validated capabilities:

""" + "\n".join(f"- {item}" for item in qr['education_family_maturity_assessment']['validated']) + "\n\nNot yet sufficient for Mature:\n\n" + "\n".join(f"- {item}" for item in qr['education_family_maturity_assessment']['insufficient_for_mature']) + "\n", encoding="utf-8")
...
- Education uses five indicator-family groups where the demographic equivalent used four.
- Education remains Stable after Campaign 26 because dedicated Education provenance-lineage completeness has not yet been executed.

Family-specific assumptions:

- Education indicator family names and matrix counts are family-specific.
...
Campaign 26 continued WDI Education family maturation without architecture change. The campaign adds inventory, territorial coverage, and temporal coverage evidence to the Education family and supports Stable status for that family.

No implementation or architecture change is justified. The next bounded campaign should execute Education provenance-lineage completeness as the family closeout campaign.
""", encoding="utf-8")
    (reports / "architectural_observations_report.md").write_text("""# Architectural Observations

Campaign 26 supports preserving the current KnowledgeForge architecture unchanged.

Supported observations:

- Education indicator-family inventory fits existing factual, classified, coverage, negative, provenance, methodological, derived, and evidence-quality categories;
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    snapshot = immutable_wdi_education_maturation_snapshot()
    ...
    quality_report = {
        **base_quality,
        ...
        "education_family_maturity_assessment": snapshot["maturity_assessment"],
        "comparison_against_campaign25": {"campaign25_accepted": 19, "campaign26_accepted": len(accepted_objects), "workflow_transfer": "unchanged", "scope_difference": "Campaign 26 adds indicator-family inventory, territorial coverage, and temporal coverage matrices to Education production."},
        ...
        "next_recommended_campaign": "Campaign 27 should execute WDI Education provenance-lineage completeness as the family closeout campaign.",
        "architectural_observations": ["Education inventory and coverage matrices fit existing taxonomy.", "No architecture, taxonomy, validator, workflow, or helper-extraction pressure appeared.", "KnowledgeForge methodology maturity judgment should wait until Education reaches Mature status."],
        "candidate_improvements_discovered": ["No implementation or architecture change is justified by Campaign 26."],
        "final_snapshot_fingerprint": final_snapshot_fingerprint,
        "final_recommendation": "Continue the WDI Education family maturation path unchanged with Education provenance-lineage completeness before classifying the Education family as Mature.",
    ...
    quality_report["repository_education_summary"] = repository_education_summary(repository_root, accepted_objects, repository_result, campaign_previous_count, quality_report["fingerprint_stability"])
    quality_report["knowledge_repository_impact_assessment"] = repository_impact_assessment(accepted_objects, quality_report["repository_education_summary"], campaign_previous_count)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Campaign 26 WDI Education maturation")
    ...
    print(json.dumps({
        ...
        "education_maturity": result["quality_report"]["education_family_maturity_assessment"]["classification"],
        "repository_fingerprint": result["quality_report"]["repository_education_summary"]["repository_fingerprint"],
        "repository_object_count": result["quality_report"]["repository_education_summary"]["total_knowledge_objects"],
