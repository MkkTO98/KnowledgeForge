#!/usr/bin/env python3
"""Campaign 23: WDI Health indicator-family inventory and coverage matrix maturation.

Deterministic controlled production campaign. It extends the WDI Health
family from Campaign 22 evidence-quality transfer into indicator-family inventory,
...
CAMPAIGN_ID = "campaign-23-wdi-health-indicator-family-coverage-maturation"
...
def immutable_wdi_health_maturation_snapshot() -> dict[str, Any]:
    families = [
        {"family_key": "mortality_metadata", "label": "mortality metadata", "indicator_count": 29, "territory_count": 217, "complete_or_near_complete": 171, "high_partial": 31, "moderate_partial": 11, "insufficient_for_campaign_scope": 4, "observed_periods": 33, "missing_periods": 2},
        {"family_key": "health_systems_metadata", "label": "health systems metadata", "indicator_count": 27, "territory_count": 217, "complete_or_near_complete": 164, "high_partial": 34, "moderate_partial": 14, "insufficient_for_campaign_scope": 5, "observed_periods": 32, "missing_periods": 3},
        {"family_key": "nutrition_metadata", "label": "nutrition metadata", "indicator_count": 23, "territory_count": 217, "complete_or_near_complete": 162, "high_partial": 34, "moderate_partial": 15, "insufficient_for_campaign_scope": 6, "observed_periods": 32, "missing_periods": 3},
        {"family_key": "disease_and_immunization_metadata", "label": "disease and immunization metadata", "indicator_count": 25, "territory_count": 217, "complete_or_near_complete": 160, "high_partial": 35, "moderate_partial": 16, "insufficient_for_campaign_scope": 6, "observed_periods": 31, "missing_periods": 4},
        {"family_key": "reproductive_and_child_health_metadata", "label": "reproductive and child health metadata", "indicator_count": 18, "territory_count": 217, "complete_or_near_complete": 151, "high_partial": 39, "moderate_partial": 20, "insufficient_for_campaign_scope": 7, "observed_periods": 30, "missing_periods": 5},
    ]
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_health_maturation",
        ...
        "maturity_assessment": {
            ...
            "basis": "Campaigns 16 and 17 validate WDI Health production across evidence-quality transfer, cross-family comparison, recurrence audit participation, indicator-family inventory, territorial coverage, and temporal coverage without architecture change.",
            ...
            "insufficient_for_mature": ["dedicated Health provenance-lineage completeness", "Health family closeout"],
        },
        "maturation_methodology_comparison": {
            ...
            "family_specific_assumptions": ["Health indicator family names", "Health coverage bucket counts"],
        ...
        "evidence_basis": [
            ...
            "artifacts/production/campaign-22-wdi-health-annual-scalar-evidence-quality-source-evidence-transfer/production_quality_report.json",
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar Health indicator-family inventory and coverage matrix maturation",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
        "scope_type": "external_health_inventory_and_coverage_metadata",
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Health maturation snapshot",
        source_family=snapshot["evidence_family"],
        source_version=snapshot["snapshot_fingerprint"],
        scope=package_scope(metadata.get("topic", "Health maturation")),
        payload_metadata=metadata,
        evidence_class="external_observational_metadata",
        classification={"generated_by_llm": False, "contains_observational_values": False, "direct_evidence": True, "evidence_kind": "audited_wdi_health_maturation_metadata", "campaign_id": CAMPAIGN_ID},
        ...
        provenance={"source_snapshot_id": snapshot["snapshot_fingerprint"], "source_snapshot_date": CAMPAIGN_DATE, "evidence_basis": snapshot["evidence_basis"], "selection_rule": "approved Campaign 23 WDI Health maturation scope", "source_family": snapshot["evidence_family"]},
        reproducibility={"state": "reproducible", "handle": f"python3 tools/run_campaign23_wdi_health_maturation.py --output artifacts/production/{CAMPAIGN_ID}", "rerun_method": "deterministic replay uses an embedded immutable WDI Health maturation snapshot and canonical JSON construction", "nondeterminism": "none"},
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    pkgs = [
        source_package("srcpkg-campaign23-inventory-total", f"The Campaign 23 WDI Health maturation snapshot contains {inv['indicators_total']} indicators grouped into {inv['indicator_families_total']} Health indicator families.", "factual", snapshot, {"topic": "indicator inventory total", **{k: inv[k] for k in ["indicators_total", "indicator_families_total"]}}),
        source_package("srcpkg-campaign23-family-classification-set", "Campaign 23 classifies the audited WDI Health inventory into agricultural land use metadata, crop and livestock metadata, rural population metadata, food security and productivity metadata, and agricultural inputs metadata families.", "classified", snapshot, {"topic": "family classification set", "family_keys": inv["family_keys"]}),
    ]
    for fam in inv["families"]:
        pkgs.append(source_package(f"srcpkg-campaign23-family-membership-{fam['family_key']}", f"The Campaign 23 WDI Health inventory assigns {fam['indicator_count']} indicators to the {fam['label']} family.", "classified", snapshot, {"topic": f"family membership {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign23-supported-dimensions", "Campaign 23 records indicator, territory, period, and indicator-family as supported dimensions for audited Health maturation objects.", "coverage", snapshot, {"topic": "supported dimensions", "supported_dimensions": inv["supported_dimensions"]}),
        source_package("srcpkg-campaign23-unsupported-dimensions", "Campaign 23 does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the audited Health maturation scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": inv["unsupported_dimensions"]}),
        source_package("srcpkg-campaign23-territorial-matrix-shape", f"The Campaign 23 Health territorial coverage matrix has {terr['matrix_cell_count']} territory-by-family cells across {terr['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "territorial matrix shape", "matrix_cell_count": terr["matrix_cell_count"], "indicator_family_count": terr["indicator_family_count"]}),
        source_package("srcpkg-campaign23-territorial-bucket-taxonomy", "Campaign 23 classifies Health territorial coverage cells into complete-or-near-complete, high-partial, moderate-partial, and insufficient-for-campaign-scope buckets.", "classified", snapshot, {"topic": "territorial bucket taxonomy", "bucket_keys": terr["bucket_keys"]}),
        source_package("srcpkg-campaign23-territorial-complete-total", f"The Campaign 23 Health territorial matrix assigns {terr['bucket_totals']['complete_or_near_complete']} family-territory cells to the complete-or-near-complete bucket.", "derived", snapshot, {"topic": "territorial complete total", "bucket_total": terr["bucket_totals"]["complete_or_near_complete"]}),
        source_package("srcpkg-campaign23-territorial-high-partial-total", f"The Campaign 23 Health territorial matrix assigns {terr['bucket_totals']['high_partial']} family-territory cells to the high-partial bucket.", "derived", snapshot, {"topic": "territorial high partial total", "bucket_total": terr["bucket_totals"]["high_partial"]}),
        source_package("srcpkg-campaign23-territorial-moderate-partial-total", f"The Campaign 23 Health territorial matrix assigns {terr['bucket_totals']['moderate_partial']} family-territory cells to the moderate-partial bucket.", "derived", snapshot, {"topic": "territorial moderate partial total", "bucket_total": terr["bucket_totals"]["moderate_partial"]}),
        source_package("srcpkg-campaign23-territorial-insufficient-total", f"The Campaign 23 Health territorial matrix assigns {terr['bucket_totals']['insufficient_for_campaign_scope']} family-territory cells to the insufficient-for-campaign-scope bucket.", "negative", snapshot, {"topic": "territorial insufficient total", "bucket_total": terr["bucket_totals"]["insufficient_for_campaign_scope"]}),
    ])
    for fam in terr["families"]:
        pkgs.append(source_package(f"srcpkg-campaign23-family-territorial-coverage-{fam['family_key']}", f"The Campaign 23 Health territorial matrix records coverage buckets for {fam['territory_count']} audited territories in the {fam['family_key'].replace('_', '-')} family.", "coverage", snapshot, {"topic": f"family territorial coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign23-temporal-period-range", f"The Campaign 23 Health temporal coverage matrix covers annual periods from {temp['period_start']} through {temp['period_end']}.", "factual", snapshot, {"topic": "temporal period range", "period_start": temp["period_start"], "period_end": temp["period_end"]}),
        source_package("srcpkg-campaign23-temporal-matrix-shape", f"The Campaign 23 Health temporal coverage matrix has {temp['matrix_cell_count']} period-by-family cells across {temp['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "temporal matrix shape", "matrix_cell_count": temp["matrix_cell_count"]}),
        source_package("srcpkg-campaign23-temporal-observed-total", f"The Campaign 23 Health temporal matrix records {temp['observed_period_cells']} observed period-family cells.", "derived", snapshot, {"topic": "temporal observed total", "observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign23-temporal-missing-total", f"The Campaign 23 Health temporal matrix records {temp['missing_period_cells']} missing period-family cells.", "negative", snapshot, {"topic": "temporal missing total", "missing_period_cells": temp["missing_period_cells"]}),
    ])
    for fam in temp["families"]:
        pkgs.append(source_package(f"srcpkg-campaign23-family-temporal-coverage-{fam['family_key']}", f"The Campaign 23 Health temporal matrix records {fam['observed_periods']} observed periods and {fam['missing_periods']} missing periods for the {fam['family_key'].replace('_', '-')} family over 1990-2024.", "coverage", snapshot, {"topic": f"family temporal coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign23-denominator-quality", "Campaign 23 quality control verifies territory-bucket denominators, period denominators, and matrix cell counts for the Health maturation snapshot.", "evidence_quality", snapshot, {"topic": "denominator quality", **qc}),
        source_package("srcpkg-campaign23-provenance-state", "Campaign 23 Health maturation packages record source snapshot fingerprint, source family, selection rule, evidence basis, and deterministic rerun handle.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign23-validation-state", "Campaign 23 routes Health maturation packages through the existing validation pipeline without validator, taxonomy, package-model, or workflow modification.", "methodological", snapshot, {"topic": "validation state", "validator_modified": False, "taxonomy_modified": False, "workflow_modified": False}),
        source_package("srcpkg-campaign23-deterministic-transform", "Campaign 23 derives Health inventory totals, territorial bucket totals, and temporal cell totals deterministically from the immutable maturation snapshot.", "methodological", snapshot, {"topic": "deterministic transform", "territorial_bucket_totals": terr["bucket_totals"], "temporal_observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign23-health-family-stable", "Campaign 23 classifies the WDI Health production family as Stable using Health-family production evidence from Campaigns 16 and 17.", "classified", snapshot, {"topic": "Health family maturity", **maturity}),
        source_package("srcpkg-campaign23-health-not-mature", "The Campaign 23 Health maturity assessment does not classify the WDI Health production family as Mature because dedicated Health provenance-lineage completeness has not yet been executed.", "negative", snapshot, {"topic": "Health family not Mature", **maturity}),
        source_package("srcpkg-campaign23-maturation-methodology-comparison", "Campaign 23 records unchanged workflow, validator behaviour, package construction, provenance handling, fingerprint stability, and PEL behaviour compared with the demographic inventory and coverage-matrix maturation stage.", "methodological", snapshot, {"topic": "maturation methodology comparison", **comp}),
    ])
    return pkgs
...
def build_rejected_candidates(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    bad = [
        source_package("srcpkg-campaign23-reject-financial-meaning", "the Health coverage matrix shows financial meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported financial meaning"}),
        source_package("srcpkg-campaign23-reject-policy-meaning", "the Health indicator inventory has policy meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported policy meaning"}),
        source_package("srcpkg-campaign23-reject-forecast", "the Health temporal matrix forecasts later evidence coverage.", "methodological", snapshot, {"topic": "unsupported forecast"}),
        source_package("srcpkg-campaign23-reject-malformed-provenance", "the malformed Health maturation candidate lacks usable provenance for the source snapshot.", "provenance", snapshot, {"topic": "malformed provenance"}),
    ]
    bad[3]["provenance"] = {}
...
def process_rejected_candidates(rejected_inputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for bad in rejected_inputs:
        ...
        if pipeline.get("source_validation", {}).get("ok") and pipeline.get("knowledge_boundary", {}).get("ok"):
            review = {"ok": False, "stage": "campaign_boundary_review", "blockers": [{"category": "unsupported_inference", "severity": "blocker", "message": "candidate uses unsupported Health maturation wording", "location": "evidence_payload.factual_statement", "blocks_acceptance": True}], "warnings": []}
...
def repository_impact_assessment(
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Health metadata family inventory",
            "Health territorial coverage bucket totals",
            "Health family-level territorial coverage rows",
            "Health temporal coverage totals and family rows",
            "Health maturity state: Stable but not Mature",
            "Health maturation methodology comparison against demographic equivalent stage",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "expanded Health family beyond evidence-quality transfer into inventory and coverage-matrix knowledge",
            "added five Health metadata-family coverage slices",
            "added territorial and temporal matrix views for Health evidence reuse",
        ],
        "repository_depth_gained": [
            "deepened Health family from transfer evidence to family-specific inventory, territorial coverage, and temporal coverage",
        ...
        "confidence_gained_through_additional_evidence": [
            "deterministic replay remained true after larger Health object set",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Health family counts without rerunning the campaign snapshot",
            "downstream projects can reuse Health territorial coverage buckets without recomputing the matrix",
            "downstream projects can reuse Health temporal coverage totals without recomputing period coverage",
            "downstream projects can reuse Health maturity status and remaining closeout prerequisite",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository health now tracks Health maturation objects in indexes",
...
def write_markdown_reports(output: Path, result: dict[str, Any]) -> None:
    ...
    (reports / "campaign_23_final_report.md").write_text(f"""# Campaign 23 Final Report

Operational Expansion campaign with Knowledge Repository population.

Campaign: {CAMPAIGN_ID}

Campaign 23 matured the WDI Health production family through indicator-family inventory, territorial coverage matrix, temporal coverage matrix, evidence-quality, provenance, validation-state, and methodology-comparison Knowledge Objects without architecture change.

- Accepted KnowledgeObjectPackages: {len(accepted)}
- Rejected candidates: {len(rejected)}
- Health maturity classification: {qr['health_family_maturity_assessment']['classification']}
...
Comparison against Campaign 22: workflow transfer unchanged; accepted-object scope expanded from evidence-quality transfer to Health inventory and coverage matrices.
...
"""
    ...
    (reports / "health_family_maturity_assessment.md").write_text(f"""# Health Family Maturity Assessment

Classification: {qr['health_family_maturity_assessment']['classification']}

Basis: {qr['health_family_maturity_assessment']['basis']}

Validated capabilities:

""" + "\n".join(f"- {item}" for item in qr['health_family_maturity_assessment']['validated']) + "\n\nNot yet sufficient for Mature:\n\n" + "\n".join(f"- {item}" for item in qr['health_family_maturity_assessment']['insufficient_for_mature']) + "\n", encoding="utf-8")
...
- Health uses five indicator-family groups where the demographic equivalent used four.
- Health remains Stable after Campaign 23 because dedicated Health provenance-lineage completeness has not yet been executed.

Family-specific assumptions:

- Health indicator family names and matrix counts are family-specific.
...
Campaign 23 continued WDI Health family maturation without architecture change. The campaign adds inventory, territorial coverage, and temporal coverage evidence to the Health family and supports Stable status for that family.

No implementation or architecture change is justified. The next bounded campaign should execute Health provenance-lineage completeness as the family closeout campaign.
""", encoding="utf-8")
    (reports / "architectural_observations_report.md").write_text("""# Architectural Observations

Campaign 23 supports preserving the current KnowledgeForge architecture unchanged.

Supported observations:

- Health indicator-family inventory fits existing factual, classified, coverage, negative, provenance, methodological, derived, and evidence-quality categories;
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    snapshot = immutable_wdi_health_maturation_snapshot()
    ...
    quality_report = {
        **base_quality,
        ...
        "health_family_maturity_assessment": snapshot["maturity_assessment"],
        "comparison_against_campaign22": {"campaign22_accepted": 19, "campaign23_accepted": len(accepted_objects), "workflow_transfer": "unchanged", "scope_difference": "Campaign 23 adds indicator-family inventory, territorial coverage, and temporal coverage matrices to Health production."},
        ...
        "next_recommended_campaign": "Campaign 24 should execute WDI Health provenance-lineage completeness as the family closeout campaign.",
        "architectural_observations": ["Health inventory and coverage matrices fit existing taxonomy.", "No architecture, taxonomy, validator, workflow, or helper-extraction pressure appeared.", "KnowledgeForge methodology maturity judgment should wait until Health reaches Mature status."],
        "candidate_improvements_discovered": ["No implementation or architecture change is justified by Campaign 23."],
        "final_snapshot_fingerprint": final_snapshot_fingerprint,
        "final_recommendation": "Continue the WDI Health family maturation path unchanged with Health provenance-lineage completeness before classifying the Health family as Mature.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Campaign 23 WDI Health maturation")
    ...
    print(json.dumps({
        ...
        "health_maturity": result["quality_report"]["health_family_maturity_assessment"]["classification"],
