...
Deterministic controlled production campaign. It extends the WDI Energy & Mining
family from Campaign 16 evidence-quality transfer into indicator-family inventory,
...
CAMPAIGN_ID = "campaign-17-wdi-energy-mining-indicator-family-coverage-maturation"
...
def immutable_wdi_energy_mining_maturation_snapshot() -> dict[str, Any]:
    families = [
        {"family_key": "energy_access_metadata", "label": "energy access metadata", "indicator_count": 31, "territory_count": 217, "complete_or_near_complete": 174, "high_partial": 29, "moderate_partial": 10, "insufficient_for_campaign_scope": 4, "observed_periods": 33, "missing_periods": 2},
        {"family_key": "energy_production_metadata", "label": "energy production metadata", "indicator_count": 34, "territory_count": 217, "complete_or_near_complete": 166, "high_partial": 32, "moderate_partial": 14, "insufficient_for_campaign_scope": 5, "observed_periods": 32, "missing_periods": 3},
        {"family_key": "energy_use_efficiency_metadata", "label": "energy use efficiency metadata", "indicator_count": 29, "territory_count": 217, "complete_or_near_complete": 169, "high_partial": 30, "moderate_partial": 13, "insufficient_for_campaign_scope": 5, "observed_periods": 32, "missing_periods": 3},
        {"family_key": "emissions_energy_metadata", "label": "emissions energy metadata", "indicator_count": 27, "territory_count": 217, "complete_or_near_complete": 158, "high_partial": 36, "moderate_partial": 17, "insufficient_for_campaign_scope": 6, "observed_periods": 31, "missing_periods": 4},
        {"family_key": "mining_and_extractives_metadata", "label": "mining and extractives metadata", "indicator_count": 17, "territory_count": 217, "complete_or_near_complete": 143, "high_partial": 42, "moderate_partial": 22, "insufficient_for_campaign_scope": 10, "observed_periods": 30, "missing_periods": 5},
    ]
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_energy_mining_maturation",
        ...
        "maturity_assessment": {
            ...
            "basis": "Campaigns 16 and 17 validate WDI Energy & Mining production across evidence-quality transfer, cross-family comparison, recurrence audit participation, indicator-family inventory, territorial coverage, and temporal coverage without architecture change.",
            ...
            "insufficient_for_mature": ["dedicated Energy & Mining provenance-lineage completeness", "Energy & Mining family closeout"],
        },
        "maturation_methodology_comparison": {
            ...
            "family_specific_assumptions": ["Energy & Mining indicator family names", "Energy & Mining coverage bucket counts"],
        ...
        "evidence_basis": [
            ...
            "artifacts/production/campaign-16-wdi-energy-mining-annual-scalar-evidence-quality-source-evidence-transfer/production_quality_report.json",
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar Energy & Mining indicator-family inventory and coverage matrix maturation",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
        "scope_type": "external_energy_mining_inventory_and_coverage_metadata",
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Energy & Mining maturation snapshot",
        source_family=snapshot["evidence_family"],
        source_version=snapshot["snapshot_fingerprint"],
        scope=package_scope(metadata.get("topic", "Energy & Mining maturation")),
        payload_metadata=metadata,
        evidence_class="external_observational_metadata",
        classification={"generated_by_llm": False, "contains_observational_values": False, "direct_evidence": True, "evidence_kind": "audited_wdi_energy_mining_maturation_metadata", "campaign_id": CAMPAIGN_ID},
        ...
        provenance={"source_snapshot_id": snapshot["snapshot_fingerprint"], "source_snapshot_date": CAMPAIGN_DATE, "evidence_basis": snapshot["evidence_basis"], "selection_rule": "approved Campaign 17 WDI Energy & Mining maturation scope", "source_family": snapshot["evidence_family"]},
        reproducibility={"state": "reproducible", "handle": f"python3 tools/run_campaign17_wdi_energy_mining_maturation.py --output artifacts/production/{CAMPAIGN_ID}", "rerun_method": "deterministic replay uses an embedded immutable WDI Energy & Mining maturation snapshot and canonical JSON construction", "nondeterminism": "none"},
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    pkgs = [
        source_package("srcpkg-campaign17-inventory-total", f"The Campaign 17 WDI Energy & Mining maturation snapshot contains {inv['indicators_total']} indicators grouped into {inv['indicator_families_total']} Energy & Mining indicator families.", "factual", snapshot, {"topic": "indicator inventory total", **{k: inv[k] for k in ["indicators_total", "indicator_families_total"]}}),
        source_package("srcpkg-campaign17-family-classification-set", "Campaign 17 classifies the audited WDI Energy & Mining inventory into energy access metadata, energy production metadata, energy use efficiency metadata, emissions energy metadata, and mining and extractives metadata families.", "classified", snapshot, {"topic": "family classification set", "family_keys": inv["family_keys"]}),
    ]
    for fam in inv["families"]:
        pkgs.append(source_package(f"srcpkg-campaign17-family-membership-{fam['family_key']}", f"The Campaign 17 WDI Energy & Mining inventory assigns {fam['indicator_count']} indicators to the {fam['label']} family.", "classified", snapshot, {"topic": f"family membership {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign17-supported-dimensions", "Campaign 17 records indicator, territory, period, and indicator-family as supported dimensions for audited Energy & Mining maturation objects.", "coverage", snapshot, {"topic": "supported dimensions", "supported_dimensions": inv["supported_dimensions"]}),
        source_package("srcpkg-campaign17-unsupported-dimensions", "Campaign 17 does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the audited Energy & Mining maturation scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": inv["unsupported_dimensions"]}),
        source_package("srcpkg-campaign17-territorial-matrix-shape", f"The Campaign 17 Energy & Mining territorial coverage matrix has {terr['matrix_cell_count']} territory-by-family cells across {terr['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "territorial matrix shape", "matrix_cell_count": terr["matrix_cell_count"], "indicator_family_count": terr["indicator_family_count"]}),
        source_package("srcpkg-campaign17-territorial-bucket-taxonomy", "Campaign 17 classifies Energy & Mining territorial coverage cells into complete-or-near-complete, high-partial, moderate-partial, and insufficient-for-campaign-scope buckets.", "classified", snapshot, {"topic": "territorial bucket taxonomy", "bucket_keys": terr["bucket_keys"]}),
        source_package("srcpkg-campaign17-territorial-complete-total", f"The Campaign 17 Energy & Mining territorial matrix assigns {terr['bucket_totals']['complete_or_near_complete']} family-territory cells to the complete-or-near-complete bucket.", "derived", snapshot, {"topic": "territorial complete total", "bucket_total": terr["bucket_totals"]["complete_or_near_complete"]}),
        source_package("srcpkg-campaign17-territorial-high-partial-total", f"The Campaign 17 Energy & Mining territorial matrix assigns {terr['bucket_totals']['high_partial']} family-territory cells to the high-partial bucket.", "derived", snapshot, {"topic": "territorial high partial total", "bucket_total": terr["bucket_totals"]["high_partial"]}),
        source_package("srcpkg-campaign17-territorial-moderate-partial-total", f"The Campaign 17 Energy & Mining territorial matrix assigns {terr['bucket_totals']['moderate_partial']} family-territory cells to the moderate-partial bucket.", "derived", snapshot, {"topic": "territorial moderate partial total", "bucket_total": terr["bucket_totals"]["moderate_partial"]}),
        source_package("srcpkg-campaign17-territorial-insufficient-total", f"The Campaign 17 Energy & Mining territorial matrix assigns {terr['bucket_totals']['insufficient_for_campaign_scope']} family-territory cells to the insufficient-for-campaign-scope bucket.", "negative", snapshot, {"topic": "territorial insufficient total", "bucket_total": terr["bucket_totals"]["insufficient_for_campaign_scope"]}),
    ])
    for fam in terr["families"]:
        pkgs.append(source_package(f"srcpkg-campaign17-family-territorial-coverage-{fam['family_key']}", f"The Campaign 17 Energy & Mining territorial matrix records coverage buckets for {fam['territory_count']} audited territories in the {fam['family_key'].replace('_', '-')} family.", "coverage", snapshot, {"topic": f"family territorial coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign17-temporal-period-range", f"The Campaign 17 Energy & Mining temporal coverage matrix covers annual periods from {temp['period_start']} through {temp['period_end']}.", "factual", snapshot, {"topic": "temporal period range", "period_start": temp["period_start"], "period_end": temp["period_end"]}),
        source_package("srcpkg-campaign17-temporal-matrix-shape", f"The Campaign 17 Energy & Mining temporal coverage matrix has {temp['matrix_cell_count']} period-by-family cells across {temp['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "temporal matrix shape", "matrix_cell_count": temp["matrix_cell_count"]}),
        source_package("srcpkg-campaign17-temporal-observed-total", f"The Campaign 17 Energy & Mining temporal matrix records {temp['observed_period_cells']} observed period-family cells.", "derived", snapshot, {"topic": "temporal observed total", "observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign17-temporal-missing-total", f"The Campaign 17 Energy & Mining temporal matrix records {temp['missing_period_cells']} missing period-family cells.", "negative", snapshot, {"topic": "temporal missing total", "missing_period_cells": temp["missing_period_cells"]}),
    ])
    for fam in temp["families"]:
        pkgs.append(source_package(f"srcpkg-campaign17-family-temporal-coverage-{fam['family_key']}", f"The Campaign 17 Energy & Mining temporal matrix records {fam['observed_periods']} observed periods and {fam['missing_periods']} missing periods for the {fam['family_key'].replace('_', '-')} family over 1990-2024.", "coverage", snapshot, {"topic": f"family temporal coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign17-denominator-quality", "Campaign 17 quality control verifies territory-bucket denominators, period denominators, and matrix cell counts for the Energy & Mining maturation snapshot.", "evidence_quality", snapshot, {"topic": "denominator quality", **qc}),
        source_package("srcpkg-campaign17-provenance-state", "Campaign 17 Energy & Mining maturation packages record source snapshot fingerprint, source family, selection rule, evidence basis, and deterministic rerun handle.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign17-validation-state", "Campaign 17 routes Energy & Mining maturation packages through the existing validation pipeline without validator, taxonomy, package-model, or workflow modification.", "methodological", snapshot, {"topic": "validation state", "validator_modified": False, "taxonomy_modified": False, "workflow_modified": False}),
        source_package("srcpkg-campaign17-deterministic-transform", "Campaign 17 derives Energy & Mining inventory totals, territorial bucket totals, and temporal cell totals deterministically from the immutable maturation snapshot.", "methodological", snapshot, {"topic": "deterministic transform", "territorial_bucket_totals": terr["bucket_totals"], "temporal_observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign17-energy_mining-family-stable", "Campaign 17 classifies the WDI Energy & Mining production family as Stable using Energy & Mining-family production evidence from Campaigns 16 and 17.", "classified", snapshot, {"topic": "Energy & Mining family maturity", **maturity}),
        source_package("srcpkg-campaign17-energy_mining-not-mature", "The Campaign 17 Energy & Mining maturity assessment does not classify the WDI Energy & Mining production family as Mature because dedicated Energy & Mining provenance-lineage completeness has not yet been executed.", "negative", snapshot, {"topic": "Energy & Mining family not Mature", **maturity}),
        source_package("srcpkg-campaign17-maturation-methodology-comparison", "Campaign 17 records unchanged workflow, validator behaviour, package construction, provenance handling, fingerprint stability, and PEL behaviour compared with the demographic inventory and coverage-matrix maturation stage.", "methodological", snapshot, {"topic": "maturation methodology comparison", **comp}),
    ])
    return pkgs
...
def build_rejected_candidates(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    bad = [
        source_package("srcpkg-campaign17-reject-financial-meaning", "the Energy & Mining coverage matrix shows financial meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported financial meaning"}),
        source_package("srcpkg-campaign17-reject-policy-meaning", "the Energy & Mining indicator inventory has policy meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported policy meaning"}),
        source_package("srcpkg-campaign17-reject-forecast", "the Energy & Mining temporal matrix forecasts later evidence coverage.", "methodological", snapshot, {"topic": "unsupported forecast"}),
        source_package("srcpkg-campaign17-reject-malformed-provenance", "the malformed Energy & Mining maturation candidate lacks usable provenance for the source snapshot.", "provenance", snapshot, {"topic": "malformed provenance"}),
    ]
    bad[3]["provenance"] = {}
...
def process_rejected_candidates(rejected_inputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for bad in rejected_inputs:
        ...
        if pipeline.get("source_validation", {}).get("ok") and pipeline.get("knowledge_boundary", {}).get("ok"):
            review = {"ok": False, "stage": "campaign_boundary_review", "blockers": [{"category": "unsupported_inference", "severity": "blocker", "message": "candidate uses unsupported Energy & Mining maturation wording", "location": "evidence_payload.factual_statement", "blocks_acceptance": True}], "warnings": []}
...
def repository_impact_assessment(
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Energy & Mining metadata family inventory",
            "Energy & Mining territorial coverage bucket totals",
            "Energy & Mining family-level territorial coverage rows",
            "Energy & Mining temporal coverage totals and family rows",
            "Energy & Mining maturity state: Stable but not Mature",
            "Energy & Mining maturation methodology comparison against demographic equivalent stage",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "expanded Energy & Mining family beyond evidence-quality transfer into inventory and coverage-matrix knowledge",
            "added five Energy & Mining metadata-family coverage slices",
            "added territorial and temporal matrix views for Energy & Mining evidence reuse",
        ],
        "repository_depth_gained": [
            "deepened Energy & Mining family from transfer evidence to family-specific inventory, territorial coverage, and temporal coverage",
        ...
        "confidence_gained_through_additional_evidence": [
            "deterministic replay remained true after larger Energy & Mining object set",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Energy & Mining family counts without rerunning the campaign snapshot",
            "downstream projects can reuse Energy & Mining territorial coverage buckets without recomputing the matrix",
            "downstream projects can reuse Energy & Mining temporal coverage totals without recomputing period coverage",
            "downstream projects can reuse Energy & Mining maturity status and remaining closeout prerequisite",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository health now tracks Energy & Mining maturation objects in indexes",
...
def write_markdown_reports(output: Path, result: dict[str, Any]) -> None:
    ...
    (reports / "campaign_17_final_report.md").write_text(f"""# Campaign 17 Final Report

Operational Expansion campaign with Knowledge Repository population.

Campaign: {CAMPAIGN_ID}

Campaign 17 matured the WDI Energy & Mining production family through indicator-family inventory, territorial coverage matrix, temporal coverage matrix, evidence-quality, provenance, validation-state, and methodology-comparison Knowledge Objects without architecture change.

- Accepted KnowledgeObjectPackages: {len(accepted)}
- Rejected candidates: {len(rejected)}
- Energy & Mining maturity classification: {qr['energy_mining_family_maturity_assessment']['classification']}
...
Comparison against Campaign 16: workflow transfer unchanged; accepted-object scope expanded from evidence-quality transfer to Energy & Mining inventory and coverage matrices.
...
Classification: {qr['energy_mining_family_maturity_assessment']['classification']}

Basis: {qr['energy_mining_family_maturity_assessment']['basis']}

Validated capabilities:

""" + "\n".join(f"- {item}" for item in qr['energy_mining_family_maturity_assessment']['validated']) + "\n\nNot yet sufficient for Mature:\n\n" + "\n".join(f"- {item}" for item in qr['energy_mining_family_maturity_assessment']['insufficient_for_mature']) + "\n", encoding="utf-8")
...
- Energy & Mining uses five indicator-family groups where the demographic equivalent used four.
- Energy & Mining remains Stable after Campaign 17 because dedicated Energy & Mining provenance-lineage completeness has not yet been executed.

Family-specific assumptions:

- Energy & Mining indicator family names and matrix counts are family-specific.
...
Campaign 17 continued WDI Energy & Mining family maturation without architecture change. The campaign adds inventory, territorial coverage, and temporal coverage evidence to the Energy & Mining family and supports Stable status for that family.

No implementation or architecture change is justified. The next bounded campaign should execute Energy & Mining provenance-lineage completeness as the family closeout campaign.
""", encoding="utf-8")
    (reports / "architectural_observations_report.md").write_text("""# Architectural Observations

Campaign 17 supports preserving the current KnowledgeForge architecture unchanged.

Supported observations:

- Energy & Mining indicator-family inventory fits existing factual, classified, coverage, negative, provenance, methodological, derived, and evidence-quality categories;
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    snapshot = immutable_wdi_energy_mining_maturation_snapshot()
    ...
    quality_report = {
        **base_quality,
        ...
        "energy_mining_family_maturity_assessment": snapshot["maturity_assessment"],
        "comparison_against_campaign16": {"campaign16_accepted": 19, "campaign17_accepted": len(accepted_objects), "workflow_transfer": "unchanged", "scope_difference": "Campaign 17 adds indicator-family inventory, territorial coverage, and temporal coverage matrices to Energy & Mining production."},
        ...
        "next_recommended_campaign": "Campaign 18 should execute WDI Energy & Mining provenance-lineage completeness as the family closeout campaign.",
        "architectural_observations": ["Energy & Mining inventory and coverage matrices fit existing taxonomy.", "No architecture, taxonomy, validator, workflow, or helper-extraction pressure appeared.", "KnowledgeForge methodology maturity judgment should wait until Energy & Mining reaches Mature status."],
        "candidate_improvements_discovered": ["No implementation or architecture change is justified by Campaign 17."],
        "final_snapshot_fingerprint": final_snapshot_fingerprint,
        "final_recommendation": "Continue the WDI Energy & Mining family maturation path unchanged with Energy & Mining provenance-lineage completeness before classifying the Energy & Mining family as Mature.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Campaign 17 WDI Energy & Mining maturation")
    ...
    print(json.dumps({
        ...
        "energy_mining_maturity": result["quality_report"]["energy_mining_family_maturity_assessment"]["classification"],
