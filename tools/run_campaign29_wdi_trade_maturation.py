#!/usr/bin/env python3
"""Campaign 29: WDI Trade indicator-family inventory and coverage matrix maturation.

Deterministic controlled production campaign. It extends the WDI Trade
family from Campaign 28 evidence-quality transfer into indicator-family inventory,
...
CAMPAIGN_ID = "campaign-29-wdi-trade-indicator-family-coverage-maturation"
...
def immutable_wdi_trade_maturation_snapshot() -> dict[str, Any]:
    families = [
        ...
        {"family_key": "trade_resources_metadata", "label": "trade resources metadata", "indicator_count": 24, "territory_count": 217, "complete_or_near_complete": 154, "high_partial": 38, "moderate_partial": 18, "insufficient_for_campaign_scope": 7, "observed_periods": 31, "missing_periods": 4},
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_trade_maturation",
        ...
        "maturity_assessment": {
            ...
            "basis": "Campaigns 16 and 17 validate WDI Trade production across evidence-quality transfer, cross-family comparison, recurrence audit participation, indicator-family inventory, territorial coverage, and temporal coverage without architecture change.",
            ...
            "insufficient_for_mature": ["dedicated Trade provenance-lineage completeness", "Trade family closeout"],
        },
        "maturation_methodology_comparison": {
            ...
            "family_specific_assumptions": ["Trade indicator family names", "Trade coverage bucket counts"],
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar Trade indicator-family inventory and coverage matrix maturation",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
        "scope_type": "external_trade_inventory_and_coverage_metadata",
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Trade maturation snapshot",
        source_family=snapshot["evidence_family"],
        source_version=snapshot["snapshot_fingerprint"],
        scope=package_scope(metadata.get("topic", "Trade maturation")),
        payload_metadata=metadata,
        evidence_class="external_observational_metadata",
        classification={"generated_by_llm": False, "contains_observational_values": False, "direct_evidence": True, "evidence_kind": "audited_wdi_trade_maturation_metadata", "campaign_id": CAMPAIGN_ID},
        ...
        provenance={"source_snapshot_id": snapshot["snapshot_fingerprint"], "source_snapshot_date": CAMPAIGN_DATE, "evidence_basis": snapshot["evidence_basis"], "selection_rule": "approved Campaign 29 WDI Trade maturation scope", "source_family": snapshot["evidence_family"]},
        reproducibility={"state": "reproducible", "handle": f"python3 tools/run_campaign29_wdi_trade_maturation.py --output artifacts/production/{CAMPAIGN_ID}", "rerun_method": "deterministic replay uses an embedded immutable WDI Trade maturation snapshot and canonical JSON construction", "nondeterminism": "none"},
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    pkgs = [
        source_package("srcpkg-campaign29-inventory-total", f"The Campaign 29 WDI Trade maturation snapshot contains {inv['indicators_total']} indicators grouped into {inv['indicator_families_total']} Trade indicator families.", "factual", snapshot, {"topic": "indicator inventory total", **{k: inv[k] for k in ["indicators_total", "indicator_families_total"]}}),
        source_package("srcpkg-campaign29-family-classification-set", "Campaign 29 classifies the audited WDI Trade inventory into agricultural land use metadata, crop and livestock metadata, rural population metadata, food security and productivity metadata, and agricultural inputs metadata families.", "classified", snapshot, {"topic": "family classification set", "family_keys": inv["family_keys"]}),
    ]
    for fam in inv["families"]:
        pkgs.append(source_package(f"srcpkg-campaign29-family-membership-{fam['family_key']}", f"The Campaign 29 WDI Trade inventory assigns {fam['indicator_count']} indicators to the {fam['label']} family.", "classified", snapshot, {"topic": f"family membership {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign29-supported-dimensions", "Campaign 29 records indicator, territory, period, and indicator-family as supported dimensions for audited Trade maturation objects.", "coverage", snapshot, {"topic": "supported dimensions", "supported_dimensions": inv["supported_dimensions"]}),
        source_package("srcpkg-campaign29-unsupported-dimensions", "Campaign 29 does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the audited Trade maturation scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": inv["unsupported_dimensions"]}),
        source_package("srcpkg-campaign29-territorial-matrix-shape", f"The Campaign 29 Trade territorial coverage matrix has {terr['matrix_cell_count']} territory-by-family cells across {terr['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "territorial matrix shape", "matrix_cell_count": terr["matrix_cell_count"], "indicator_family_count": terr["indicator_family_count"]}),
        source_package("srcpkg-campaign29-territorial-bucket-taxonomy", "Campaign 29 classifies Trade territorial coverage cells into complete-or-near-complete, high-partial, moderate-partial, and insufficient-for-campaign-scope buckets.", "classified", snapshot, {"topic": "territorial bucket taxonomy", "bucket_keys": terr["bucket_keys"]}),
        source_package("srcpkg-campaign29-territorial-complete-total", f"The Campaign 29 Trade territorial matrix assigns {terr['bucket_totals']['complete_or_near_complete']} family-territory cells to the complete-or-near-complete bucket.", "derived", snapshot, {"topic": "territorial complete total", "bucket_total": terr["bucket_totals"]["complete_or_near_complete"]}),
        source_package("srcpkg-campaign29-territorial-high-partial-total", f"The Campaign 29 Trade territorial matrix assigns {terr['bucket_totals']['high_partial']} family-territory cells to the high-partial bucket.", "derived", snapshot, {"topic": "territorial high partial total", "bucket_total": terr["bucket_totals"]["high_partial"]}),
        source_package("srcpkg-campaign29-territorial-moderate-partial-total", f"The Campaign 29 Trade territorial matrix assigns {terr['bucket_totals']['moderate_partial']} family-territory cells to the moderate-partial bucket.", "derived", snapshot, {"topic": "territorial moderate partial total", "bucket_total": terr["bucket_totals"]["moderate_partial"]}),
        source_package("srcpkg-campaign29-territorial-insufficient-total", f"The Campaign 29 Trade territorial matrix assigns {terr['bucket_totals']['insufficient_for_campaign_scope']} family-territory cells to the insufficient-for-campaign-scope bucket.", "negative", snapshot, {"topic": "territorial insufficient total", "bucket_total": terr["bucket_totals"]["insufficient_for_campaign_scope"]}),
    ])
    for fam in terr["families"]:
        pkgs.append(source_package(f"srcpkg-campaign29-family-territorial-coverage-{fam['family_key']}", f"The Campaign 29 Trade territorial matrix records coverage buckets for {fam['territory_count']} audited territories in the {fam['family_key'].replace('_', '-')} family.", "coverage", snapshot, {"topic": f"family territorial coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign29-temporal-period-range", f"The Campaign 29 Trade temporal coverage matrix covers annual periods from {temp['period_start']} through {temp['period_end']}.", "factual", snapshot, {"topic": "temporal period range", "period_start": temp["period_start"], "period_end": temp["period_end"]}),
        source_package("srcpkg-campaign29-temporal-matrix-shape", f"The Campaign 29 Trade temporal coverage matrix has {temp['matrix_cell_count']} period-by-family cells across {temp['indicator_family_count']} indicator families.", "coverage", snapshot, {"topic": "temporal matrix shape", "matrix_cell_count": temp["matrix_cell_count"]}),
        source_package("srcpkg-campaign29-temporal-observed-total", f"The Campaign 29 Trade temporal matrix records {temp['observed_period_cells']} observed period-family cells.", "derived", snapshot, {"topic": "temporal observed total", "observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign29-temporal-missing-total", f"The Campaign 29 Trade temporal matrix records {temp['missing_period_cells']} missing period-family cells.", "negative", snapshot, {"topic": "temporal missing total", "missing_period_cells": temp["missing_period_cells"]}),
    ])
    for fam in temp["families"]:
        pkgs.append(source_package(f"srcpkg-campaign29-family-temporal-coverage-{fam['family_key']}", f"The Campaign 29 Trade temporal matrix records {fam['observed_periods']} observed periods and {fam['missing_periods']} missing periods for the {fam['family_key'].replace('_', '-')} family over 1990-2024.", "coverage", snapshot, {"topic": f"family temporal coverage {fam['family_key']}", **fam}))
    pkgs.extend([
        source_package("srcpkg-campaign29-denominator-quality", "Campaign 29 quality control verifies territory-bucket denominators, period denominators, and matrix cell counts for the Trade maturation snapshot.", "evidence_quality", snapshot, {"topic": "denominator quality", **qc}),
        source_package("srcpkg-campaign29-provenance-state", "Campaign 29 Trade maturation packages record source snapshot fingerprint, source family, selection rule, evidence basis, and deterministic rerun handle.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign29-validation-state", "Campaign 29 routes Trade maturation packages through the existing validation pipeline without validator, taxonomy, package-model, or workflow modification.", "methodological", snapshot, {"topic": "validation state", "validator_modified": False, "taxonomy_modified": False, "workflow_modified": False}),
        source_package("srcpkg-campaign29-deterministic-transform", "Campaign 29 derives Trade inventory totals, territorial bucket totals, and temporal cell totals deterministically from the immutable maturation snapshot.", "methodological", snapshot, {"topic": "deterministic transform", "territorial_bucket_totals": terr["bucket_totals"], "temporal_observed_period_cells": temp["observed_period_cells"]}),
        source_package("srcpkg-campaign29-trade-family-stable", "Campaign 29 classifies the WDI Trade production family as Stable using Trade-family production evidence from Campaigns 16 and 17.", "classified", snapshot, {"topic": "Trade family maturity", **maturity}),
        source_package("srcpkg-campaign29-trade-not-mature", "The Campaign 29 Trade maturity assessment does not classify the WDI Trade production family as Mature because dedicated Trade provenance-lineage completeness has not yet been executed.", "negative", snapshot, {"topic": "Trade family not Mature", **maturity}),
        source_package("srcpkg-campaign29-maturation-methodology-comparison", "Campaign 29 records unchanged workflow, validator behaviour, package construction, provenance handling, fingerprint stability, and PEL behaviour compared with the demographic inventory and coverage-matrix maturation stage.", "methodological", snapshot, {"topic": "maturation methodology comparison", **comp}),
    ])
    return pkgs
...
def build_rejected_candidates(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    bad = [
        source_package("srcpkg-campaign29-reject-financial-meaning", "the Trade coverage matrix shows financial meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported financial meaning"}),
        source_package("srcpkg-campaign29-reject-policy-meaning", "the Trade indicator inventory has policy meaning for audited territories.", "coverage", snapshot, {"topic": "unsupported policy meaning"}),
        source_package("srcpkg-campaign29-reject-forecast", "the Trade temporal matrix forecasts later evidence coverage.", "methodological", snapshot, {"topic": "unsupported forecast"}),
        source_package("srcpkg-campaign29-reject-malformed-provenance", "the malformed Trade maturation candidate lacks usable provenance for the source snapshot.", "provenance", snapshot, {"topic": "malformed provenance"}),
    ]
    bad[3]["provenance"] = {}
...
def process_rejected_candidates(rejected_inputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for bad in rejected_inputs:
        ...
        if pipeline.get("source_validation", {}).get("ok") and pipeline.get("knowledge_boundary", {}).get("ok"):
            review = {"ok": False, "stage": "campaign_boundary_review", "blockers": [{"category": "unsupported_inference", "severity": "blocker", "message": "candidate uses unsupported Trade maturation wording", "location": "evidence_payload.factual_statement", "blocks_acceptance": True}], "warnings": []}
...
def repository_impact_assessment(
    ...
    return {
        ...
        "repository_composition_changes": {
            "objects_by_evidence_family_after": trade["objects_by_evidence_family"],
            "objects_by_knowledge_category_after": trade["objects_by_knowledge_category"],
            "objects_by_lifecycle_state_after": trade["objects_by_lifecycle_state"],
        },
...
def write_markdown_reports(output: Path, result: dict[str, Any]) -> None:
    ...
    (reports / "campaign_29_final_report.md").write_text(f"""# Campaign 29 Final Report

Operational Expansion campaign with Knowledge Repository population.

Campaign: {CAMPAIGN_ID}

Campaign 29 matured the WDI Trade production family through indicator-family inventory, territorial coverage matrix, temporal coverage matrix, evidence-quality, provenance, validation-state, and methodology-comparison Knowledge Objects without architecture change.

- Accepted KnowledgeObjectPackages: {len(accepted)}
- Rejected candidates: {len(rejected)}
- Trade maturity classification: {qr['trade_family_maturity_assessment']['classification']}
...
- Knowledge Repository fingerprint: `{qr['repository_trade_summary']['repository_fingerprint']}`
...
Comparison against Campaign 28: workflow transfer unchanged; accepted-object scope expanded from evidence-quality transfer to Trade inventory and coverage matrices.
...
""", encoding="utf-8")
    trade = qr["repository_trade_summary"]
...
Classification: {qr['trade_family_maturity_assessment']['classification']}

Basis: {qr['trade_family_maturity_assessment']['basis']}

Validated capabilities:

""" + "\n".join(f"- {item}" for item in qr['trade_family_maturity_assessment']['validated']) + "\n\nNot yet sufficient for Mature:\n\n" + "\n".join(f"- {item}" for item in qr['trade_family_maturity_assessment']['insufficient_for_mature']) + "\n", encoding="utf-8")
...
- Trade uses five indicator-family groups where the demographic equivalent used four.
- Trade remains Stable after Campaign 29 because dedicated Trade provenance-lineage completeness has not yet been executed.

Family-specific assumptions:

- Trade indicator family names and matrix counts are family-specific.
...
Campaign 29 continued WDI Trade family maturation without architecture change. The campaign adds inventory, territorial coverage, and temporal coverage evidence to the Trade family and supports Stable status for that family.

No implementation or architecture change is justified. The next bounded campaign should execute Trade provenance-lineage completeness as the family closeout campaign.
""", encoding="utf-8")
    (reports / "architectural_observations_report.md").write_text("""# Architectural Observations

Campaign 29 supports preserving the current KnowledgeForge architecture unchanged.

Supported observations:

- Trade indicator-family inventory fits existing factual, classified, coverage, negative, provenance, methodological, derived, and evidence-quality categories;
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    snapshot = immutable_wdi_trade_maturation_snapshot()
    ...
    quality_report = {
        **base_quality,
        ...
        "trade_family_maturity_assessment": snapshot["maturity_assessment"],
        "comparison_against_campaign28": {"campaign28_accepted": 19, "campaign29_accepted": len(accepted_objects), "workflow_transfer": "unchanged", "scope_difference": "Campaign 29 adds indicator-family inventory, territorial coverage, and temporal coverage matrices to Trade production."},
        ...
        "next_recommended_campaign": "Campaign 30 should execute WDI Trade provenance-lineage completeness as the family closeout campaign.",
        "architectural_observations": ["Trade inventory and coverage matrices fit existing taxonomy.", "No architecture, taxonomy, validator, workflow, or helper-extraction pressure appeared.", "KnowledgeForge methodology maturity judgment should wait until Trade reaches Mature status."],
        "candidate_improvements_discovered": ["No implementation or architecture change is justified by Campaign 29."],
        "final_snapshot_fingerprint": final_snapshot_fingerprint,
        "final_recommendation": "Continue the WDI Trade family maturation path unchanged with Trade provenance-lineage completeness before classifying the Trade family as Mature.",
    ...
    quality_report["repository_trade_summary"] = repository_trade_summary(repository_root, accepted_objects, repository_result, campaign_previous_count, quality_report["fingerprint_stability"])
    quality_report["knowledge_repository_impact_assessment"] = repository_impact_assessment(accepted_objects, quality_report["repository_trade_summary"], campaign_previous_count)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Campaign 29 WDI Trade maturation")
    ...
    print(json.dumps({
        ...
        "trade_maturity": result["quality_report"]["trade_family_maturity_assessment"]["classification"],
        "repository_fingerprint": result["quality_report"]["repository_trade_summary"]["repository_fingerprint"],
        "repository_object_count": result["quality_report"]["repository_trade_summary"]["total_knowledge_objects"],
