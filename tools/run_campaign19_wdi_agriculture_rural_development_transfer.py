#!/usr/bin/env python3
"""Campaign 19: WDI Agriculture & Rural Development annual-scalar evidence-quality and source-evidence transfer.

Deterministic controlled production campaign. It uses an embedded immutable
WDI Agriculture & Rural Development annual-scalar evidence snapshot and the existing KnowledgeForge
...
CAMPAIGN_ID = "campaign-19-wdi-agriculture-rural-development-annual-scalar-evidence-quality-source-evidence-transfer"
...
def immutable_wdi_agriculture_rural_development_snapshot() -> dict[str, Any]:
    """Return the bounded immutable WDI Agriculture & Rural Development evidence snapshot.
    ...
    """
    family_rows = [
        {"family": "agricultural_land_use_metadata", "indicator_count": 29, "observed_series_cells": 137460, "missing_series_cells": 31540, "applicability": "annual scalar indicator metadata"},
        {"family": "crop_and_livestock_metadata", "indicator_count": 27, "observed_series_cells": 129880, "missing_series_cells": 28420, "applicability": "annual scalar indicator metadata"},
        {"family": "rural_population_metadata", "indicator_count": 24, "observed_series_cells": 121720, "missing_series_cells": 19160, "applicability": "annual scalar indicator metadata"},
        {"family": "agricultural_inputs_metadata", "indicator_count": 23, "observed_series_cells": 109330, "missing_series_cells": 33570, "applicability": "annual scalar indicator metadata"},
        {"family": "food_security_and_productivity_metadata", "indicator_count": 23, "observed_series_cells": 112450, "missing_series_cells": 30620, "applicability": "annual scalar indicator metadata"},
    ]
    ...
    snapshot = {
        ...
        "evidence_family": "external_wdi_annual_scalar_agriculture_rural_development",
        "scope": {
            ...
            "indicator_scope": "WDI Agriculture & Rural Development topic annual-scalar indicators selected by the Phase 2 Production Roadmap",
        },
        "audited_inventory": {
            "source": "World Bank World Development Indicators (WDI)",
            "wdi_topic_id": 1,
            "wdi_topic_name": "Agriculture & Rural Development",
            ...
            "dataset_releases": 3,
            "pipeline_runs": 8,
            "selection_source": "Campaign 19 planning gate and Phase 2 Production Roadmap",
        },
        "agriculture_rural_development_family_inventory": {
        ...
        "evidence_basis": [
            ...
            "artifacts/production/campaign-18-wdi-energy-mining-provenance-lineage-closeout/production_quality_report.json",
            "artifacts/production/campaign-18-wdi-energy-mining-provenance-lineage-closeout/reports/family_closeout_report.md",
        ],
    }
...
def package_scope(topic: str) -> dict[str, Any]:
    return {
        "domain": "WDI annual-scalar agriculture and rural development evidence-quality and source-evidence transfer",
        "campaign_id": CAMPAIGN_ID,
        "topic": topic,
...
def source_package(package_id: str, statement: str, category: str, snapshot: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    return production_support.build_source_evidence_package(
        ...
        source_name="World Bank World Development Indicators audited Agriculture & Rural Development evidence snapshot",
        ...
        classification={
            ...
            "evidence_kind": "audited_wdi_agriculture_rural_development_snapshot_characteristic",
            "campaign_id": CAMPAIGN_ID,
        },
        validation_metadata={
        ...
        provenance={
            ...
            "selection_rule": "Phase 2 Production Roadmap selected WDI Agriculture & Rural Development annual-scalar evidence as the next production family after WDI Energy & Mining reached Mature",
            "source_family": snapshot["evidence_family"],
        },
        reproducibility={
            "state": "reproducible",
            "handle": f"python3 tools/run_campaign19_wdi_agriculture_rural_development_transfer.py --output artifacts/production/{CAMPAIGN_ID}",
            "rerun_method": "deterministic embedded immutable WDI Agriculture & Rural Development evidence snapshot and canonical JSON construction",
            "nondeterminism": "none",
        },
...
def build_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    fam = snapshot["agriculture_rural_development_family_inventory"]
    ...
    packages = [
        source_package("srcpkg-campaign19-agriculture_rural_development-source-scope", "The Campaign 19 source evidence package is scoped to World Bank WDI annual-scalar Agriculture & Rural Development evidence.", "factual", snapshot, {"topic": "source scope", "source": inv["source"], "frequency": snapshot["scope"]["frequency"], "shape": snapshot["scope"]["shape"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-topic-inventory", f"The audited WDI Agriculture & Rural Development topic inventory records {inv['indicators_total']} annual-scalar indicators.", "coverage", snapshot, {"topic": "indicator coverage", "indicator_count": inv["indicators_total"], "wdi_topic_id": inv["wdi_topic_id"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-territory-scope", f"Campaign 19 uses the same audited WDI territory scope count of {inv['territories_total']} non-aggregate or country-like territories.", "coverage", snapshot, {"topic": "territory coverage", "territory_count": inv["territories_total"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-period-scope", f"Campaign 19 uses {inv['period_count']} annual periods from {inv['period_start']} through {inv['period_end']} for Agriculture & Rural Development evidence coverage.", "coverage", snapshot, {"topic": "temporal coverage", "period_start": inv["period_start"], "period_end": inv["period_end"], "period_count": inv["period_count"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-observed-missing", f"The Campaign 19 Agriculture & Rural Development snapshot records {inv['observed_cells']} observed cells and {inv['explicit_missing_cells']} explicit missing cells.", "coverage", snapshot, {"topic": "observed missing coverage", "observed_cells": inv["observed_cells"], "explicit_missing_cells": inv["explicit_missing_cells"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-observed-share", f"The Campaign 19 Agriculture & Rural Development snapshot has observed cell share {metrics['observed_share']} within the audited evidence grid.", "derived", snapshot, {"topic": "observed share", "observed_share": metrics["observed_share"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-missing-share", f"The Campaign 19 Agriculture & Rural Development snapshot has explicit missing cell share {metrics['missing_share']} within the audited evidence grid.", "negative", snapshot, {"topic": "missing share", "missing_share": metrics["missing_share"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-family-count", f"The Campaign 19 Agriculture & Rural Development inventory groups the selected indicators into {fam['family_count']} deterministic metadata families.", "classified", snapshot, {"topic": "family count", "family_count": fam["family_count"], "families": [r["family"] for r in fam["families"]]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-partial-families", f"Campaign 19 records {metrics['partial_family_count']} Agriculture & Rural Development metadata families with explicit missing cells in the audited evidence grid.", "negative", snapshot, {"topic": "partial family count", "partial_family_count": metrics["partial_family_count"]}),
        source_package("srcpkg-campaign19-agriculture_rural_development-provenance-state", "Campaign 19 records WDI Agriculture & Rural Development source URLs, raw artifact URLs, raw artifact hashes, release keys, license notes, and WDI last-updated metadata as available in the evidence snapshot.", "provenance", snapshot, {"topic": "provenance state", **prov}),
        source_package("srcpkg-campaign19-agriculture_rural_development-evidence-quality-state", "Campaign 19 records explicit observation status, observed and missing status, WDI-only source scope, and annual-scalar evidence shape for the Agriculture & Rural Development family.", "evidence_quality", snapshot, {"topic": "evidence quality state", **val}),
        source_package("srcpkg-campaign19-agriculture_rural_development-unsupported-dimensions", "The Campaign 19 Agriculture & Rural Development annual-scalar evidence snapshot does not include subnational-region, scenario, model-projection, or narrative-assessment dimensions in the accepted production scope.", "negative", snapshot, {"topic": "unsupported dimensions", "unsupported_dimensions": fam["unsupported_dimensions"]}),
        source_package("srcpkg-campaign19-replication-contract", "Campaign 19 uses unchanged production workflow, package construction, validator behaviour, Production Support, provenance handling, fingerprinting, reporting, metrics, and Production Evolution Log workflow from the mature WDI demographic production method.", "methodological", snapshot, {"topic": "replication contract", **repl}),
        source_package("srcpkg-campaign19-deterministic-transfer-method", "Campaign 19 derives Agriculture & Rural Development evidence-quality and coverage objects deterministically from the immutable Agriculture & Rural Development evidence snapshot.", "methodological", snapshot, {"topic": "deterministic transfer method", "input_snapshot": snapshot["snapshot_fingerprint"]}),
    ]
    for row in fam["families"]:
        packages.append(source_package(
            f"srcpkg-campaign19-family-{row['family'].replace('_', '-')}",
            f"Campaign 19 records {row['indicator_count']} annual-scalar indicators for the {row['family']} Agriculture & Rural Development metadata family.",
            "coverage",
            snapshot,
            {"topic": "agriculture and rural development metadata family", **row},
        ))
    return packages
...
def build_rejected_source_packages(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    rejected = []
    bad_boundary = source_package("srcpkg-campaign19-reject-boundary-language", "This Agriculture & Rural Development candidate includes a causal claim, a forecast, and a recommendation.", "coverage", snapshot, {"topic": "boundary rejection"})
    rejected.append(bad_boundary)
    missing_provenance = source_package("srcpkg-campaign19-reject-missing-provenance", "A malformed Campaign 19 candidate omits provenance fields.", "provenance", snapshot, {"topic": "malformed provenance"})
    missing_provenance.pop("provenance")
    rejected.append(missing_provenance)
    missing_fingerprint = source_package("srcpkg-campaign19-reject-missing-fingerprint", "A malformed Campaign 19 candidate omits lineage fingerprints.", "provenance", snapshot, {"topic": "malformed fingerprint"})
    missing_fingerprint.pop("fingerprints")
    rejected.append(missing_fingerprint)
    unsupported_category = source_package("srcpkg-campaign19-reject-unsupported-category", "A malformed Campaign 19 candidate uses an unsupported energy-mining-rating category.", "agriculture_rural_development_rating", snapshot, {"topic": "unsupported category"})
...
def repository_impact_assessment(accepted_objects: list[dict[str, Any]], health: dict[str, Any], previous_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "new_reusable_knowledge_introduced": [
            "Agriculture & Rural Development source scope and WDI annual-scalar evidence-shape confirmation",
            "Agriculture & Rural Development indicator inventory and metadata-family counts",
            "Agriculture & Rural Development observed/missing evidence-grid totals and shares",
            "Agriculture & Rural Development provenance-state and validation-state facts",
            "Agriculture & Rural Development unsupported-dimension negative knowledge",
        ],
        "knowledge_categories_expanded": categories,
        "evidence_family_coverage_expanded": evidence_families,
        "repository_breadth_gained": [
            "opened WDI Agriculture & Rural Development as the next Phase 2 production family",
        ...
        "repository_depth_gained": [
            "added family-entry evidence-quality/source-evidence baseline for Agriculture & Rural Development maturation",
            "preserved source/provenance/validation evidence needed for later Agriculture & Rural Development inventory and provenance-lineage closeout",
        ...
        "future_recomputation_avoided_for_downstream_projects": [
            "downstream projects can reuse Agriculture & Rural Development source-scope and indicator-count facts without rerunning the campaign snapshot",
            "downstream projects can reuse Agriculture & Rural Development observed/missing evidence-grid totals and shares",
            "downstream projects can reuse Agriculture & Rural Development unsupported-dimension and provenance-state facts",
            "future Agriculture & Rural Development maturation campaigns can build on this family-entry baseline",
        ],
        ...
        "repository_quality_improvements_achieved": [
            "repository health now tracks Agriculture & Rural Development family-entry objects in indexes",
...
def run_campaign(output: Path, repository_root: Path | None = None) -> dict[str, Any]:
    ...
    snapshot = immutable_wdi_agriculture_rural_development_snapshot()
    ...
    replay_snapshot = immutable_wdi_agriculture_rural_development_snapshot()
    ...
    comparison["campaign_19"] = {"accepted": len(accepted), "rejected": len(rejected), "determinism": determinism_verified, "fingerprint_stability": fingerprint_stability, "duplicates": duplicate_detected}
    ...
    transfer = {
        ...
        "campaign19_accepted": len(accepted),
        "campaign1_rejected": campaign1.get("rejected"),
        "campaign19_rejected": len(rejected),
        "observed_differences": [
            {"difference": "Agriculture & Rural Development source family replaces demographic source family while preserving WDI annual-scalar shape.", "classification": "Agriculture & Rural Development-specific"},
            {"difference": "Agriculture & Rural Development accepted object count differs from Campaign 1 because Campaign 19 includes metadata-family coverage rows.", "classification": "evidence-family-specific"},
    ...
    quality.update({
        ...
        "processing_statistics": {
            ...
            "agriculture_rural_development_indicator_count": snapshot["audited_inventory"]["indicators_total"],
            "agriculture_rural_development_family_count": snapshot["agriculture_rural_development_family_inventory"]["family_count"],
            "observed_share": snapshot["coverage_metrics"]["observed_share"],
            "missing_share": snapshot["coverage_metrics"]["missing_share"],
        },
        "architectural_observations": [
            "Campaign 19 replicated the mature WDI demographic production methodology for WDI Agriculture & Rural Development evidence without architecture change.",
            ...
            "Campaign 19 does not exercise multi-reference accepted objects; Agriculture & Rural Development maturation remains the appropriate continuation.",
        ],
        "candidate_improvements_discovered": [
            "No validator, taxonomy, helper extraction, or architecture change is justified by Campaign 19.",
            "PEL-012 remains monitor because accepted Campaign 19 objects still use one evidence reference each.",
    ...
    quality["final_recommendation"] = "Continue maturing the WDI Agriculture & Rural Development annual-scalar family under Operational Expansion; preserve the existing KnowledgeForge production methodology unchanged and populate the Knowledge Repository after validation."
    ...
    summary = {
        ...
        "final_recommendation": "Continue maturing the WDI Agriculture & Rural Development annual-scalar family under Operational Expansion; preserve the existing KnowledgeForge production methodology unchanged and populate the Knowledge Repository after validation.",
...
def write_reports(output: Path, summary: dict[str, Any], quality: dict[str, Any], catalogue: list[dict[str, Any]], rejected: list[dict[str, Any]]) -> None:
    ...
    write_text(reports / "campaign_19_final_report.md", f"""
# Campaign 19 Final Report
...
Campaign 19 accepted {summary['accepted']} KnowledgeObjectPackages and preserved {summary['rejected']} rejected candidates.

The mature WDI demographic production methodology transferred to WDI Agriculture & Rural Development evidence without architecture, taxonomy, validator, package-model, Production Support, provenance, fingerprint, reporting, or PEL workflow modification.
...
Campaign 19 matches Campaign 0, Campaign 1, and the mature demographic family on deterministic replay, fingerprint stability, provenance completeness, rejected-candidate preservation, and absence of observed duplicate pressure.
""")
    write_text(reports / "replication_assessment.md", f"""
# Replication Assessment

Campaign 19 transfers the established production methodology to WDI Agriculture & Rural Development annual-scalar evidence after three Mature WDI families.
...
- Campaign 19 accepted: {transfer['campaign19_accepted']}
- Campaign 1 rejected: {transfer['campaign1_rejected']}
- Campaign 19 rejected: {transfer['campaign19_rejected']}

The accepted-object count differs because Campaign 19 includes Agriculture & Rural Development metadata-family coverage rows. This difference is evidence-family-specific and does not imply architecture pressure.
""")
...
The mature WDI demographic production methodology transferred successfully to WDI Agriculture & Rural Development annual-scalar evidence.
...
- Agriculture & Rural Development-specific: caused by the selected Agriculture & Rural Development family itself.
...
## What Campaign 19 showed

- The established WDI annual-scalar production methodology transferred to WDI Agriculture & Rural Development after three Mature WDI families.
...
- Agriculture & Rural Development evidence produced valid evidence-quality, coverage, factual, classified, derived, negative, provenance, and methodological objects.
...
Proceed to WDI Agriculture & Rural Development maturation unchanged unless a Doctrine Review Trigger occurs.
""")
...
1. Current package and validator contracts handled WDI Agriculture & Rural Development evidence-quality and coverage transfer without modification.
2. Existing taxonomy covered Agriculture & Rural Development evidence-level production objects.
...
5. PEL-012 remains open because accepted Campaign 19 objects still use one evidence reference each.
6. No architecture redesign is supported.

## Unsupported changes

Campaign 19 does not support ontology/taxonomy change, validator modification, repository coupling, adapter/API/shared-schema work, database coupling, runtime infrastructure, local-model generation, frontier-model generation, or additional helper extraction.
""")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Campaign 19 WDI Agriculture & Rural Development transfer production campaign")
