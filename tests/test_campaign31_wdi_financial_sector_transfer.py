...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign31_wdi_financial_sector_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign31_wdi_financial_sector_transfer", SCRIPT_PATH)
...
class Campaign22WDIFinancialSectorTransferTests(unittest.TestCase):
    def test_campaign31_snapshot_is_financial_sector_annual_scalar_scope(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_financial_sector_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_financial_sector")
    ...
    def test_campaign31_builds_financial_sector_packages_without_boundary_terms(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_financial_sector_snapshot()
    ...
    def test_campaign31_run_is_deterministic_and_populates_repository_with_required_closeout(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign31"
            ...
            self.assertTrue((output / "reports" / "repository_financial_sector_summary.md").exists())
        ...
        self.assertEqual(quality1["repository_financial_sector_summary"]["total_knowledge_objects"], 19)
    ...
    def test_campaign31_reports_operational_expansion_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign31"
            repo = Path(tmp) / "knowledge_repository"
            summary = campaign.run_campaign(output, repository_root=repo)
            final = (output / "reports" / "campaign_31_final_report.md").read_text()
        ...
        self.assertIn("Financial Sector-specific", transfer)
        ...
        self.assertIn("Financial Sector", impact)
