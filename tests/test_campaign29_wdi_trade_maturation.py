...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign29_wdi_trade_maturation.py"
CAMPAIGN19_PATH = PROJECT_ROOT / "tools" / "run_campaign28_wdi_trade_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign29_wdi_trade_maturation", SCRIPT_PATH)
...
def load_campaign28_module():
    spec = importlib.util.spec_from_file_location("run_campaign28_wdi_trade_transfer", CAMPAIGN19_PATH)
    if spec is None or spec.loader is None:
...
class Campaign23WDITradeMaturationTests(unittest.TestCase):
    def test_trade_maturation_snapshot_has_inventory_territorial_and_temporal_matrices(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_trade_maturation_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_trade_maturation")
    ...
    def test_campaign29_generates_valid_trade_maturation_objects_and_trade_summary(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_trade_maturation_snapshot()
        ...
        self.assertIn("trade", promoted_text)

    def test_campaign29_is_deterministic_preserves_rejections_and_populates_repository_after_campaign28(self) -> None:
        campaign = load_campaign_module()
        campaign28 = load_campaign28_module()
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp) / "knowledge_repository"
            campaign28.run_campaign(Path(tmp) / "campaign28", repository_root=repo)
            output = Path(tmp) / "campaign29"
            ...
            trade_exists = (output / "reports" / "repository_trade_summary.md").exists()
        ...
        self.assertTrue(trade_exists)
        self.assertTrue(impact_exists)

    def test_trade_stable_not_mature_and_doctrine_preserved(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            summary = campaign.run_campaign(Path(tmp) / "campaign29", repository_root=Path(tmp) / "knowledge_repository")
            quality = summary["quality_report"]
            final = (Path(tmp) / "campaign29" / "reports" / "campaign_29_final_report.md").read_text()
        self.assertEqual(quality["trade_family_maturity_assessment"]["classification"], "Stable")
