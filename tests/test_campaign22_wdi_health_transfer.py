...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign22_wdi_health_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign22_wdi_health_transfer", SCRIPT_PATH)
...
class Campaign22WDIHealthTransferTests(unittest.TestCase):
    def test_campaign22_snapshot_is_health_annual_scalar_scope(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_health_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_health")
        self.assertEqual(snapshot["scope"]["domain_family"], "health evidence")
        self.assertEqual(snapshot["audited_inventory"]["indicators_total"], 110)
        self.assertEqual(snapshot["scope"]["frequency"], "annual")
        self.assertEqual(snapshot["scope"]["shape"], "scalar observations")

    def test_campaign22_builds_health_packages_without_boundary_terms(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_health_snapshot()
    ...
    def test_campaign22_run_is_deterministic_and_populates_repository_with_required_closeout(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign22"
    ...
    def test_campaign22_reports_operational_expansion_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign22"
            repo = Path(tmp) / "knowledge_repository"
            summary = campaign.run_campaign(output, repository_root=repo)
            final = (output / "reports" / "campaign_22_final_report.md").read_text()
        ...
        self.assertIn("Health-specific", transfer)
