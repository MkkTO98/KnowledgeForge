...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign3_wdi_freshness_metadata.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign3_wdi_freshness_metadata", SCRIPT_PATH)
...
class Campaign3WdiFreshnessMetadataTests(unittest.TestCase):
    def setUp(self):
        self.module = load_campaign_module()

    def test_snapshot_is_deterministic_and_freshness_scoped(self):
        first = self.module.immutable_wdi_freshness_snapshot()
        second = self.module.immutable_wdi_freshness_snapshot()
        self.assertEqual(first, second)
        self.assertTrue(first["snapshot_fingerprint"].startswith("sha256:"))
        self.assertEqual(first["freshness_metadata"]["dataset_release_count"], 3)
        self.assertEqual(first["freshness_metadata"]["pipeline_run_count"], 8)
        self.assertEqual(first["freshness_metadata"]["last_updated_coverage"], 1.0)
        self.assertEqual(first["freshness_metadata"]["release_key_coverage"], 1.0)
        self.assertEqual(first["freshness_metadata"]["release_date_coverage"], 0.0)

    def test_campaign_produces_valid_freshness_metadata_objects(self):
        with tempfile.TemporaryDirectory() as td:
            output = Path(td) / "campaign3"
            ...
            self.assertIn("campaign_3", quality["cross_campaign_metric_comparison"])
