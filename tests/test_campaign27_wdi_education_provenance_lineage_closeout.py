...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign27_wdi_education_provenance_lineage_closeout.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign27_wdi_education_provenance_lineage_closeout", SCRIPT_PATH)
...
class Campaign24WDIEducationProvenanceLineageCloseoutTests(unittest.TestCase):
    def test_snapshot_is_education_provenance_lineage_scoped_and_architecture_preserving(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_education_provenance_lineage_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_education_provenance_lineage")
        self.assertEqual(snapshot["scope"]["domain_family"], "education evidence")
        self.assertTrue(snapshot["education_family_maturity"]["classification"] == "Mature")
        self.assertFalse(snapshot["architectural_continuity_review"]["refinements_supported"])

    def test_campaign27_produces_education_provenance_lineage_closeout_and_mature_family(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign27"
            ...
            family_report = (output / "reports" / "education_family_closeout_report.md").read_text()
        ...
        self.assertEqual(quality1["education_family_maturity_assessment"]["classification"], "Mature")
        ...
        self.assertIn("WDI Education Production Family Closeout Report", family_report)
        self.assertIn("preserved across seven Mature WDI annual-scalar production families", method_report)
    ...
    def test_final_recommendation_moves_to_next_roadmap_family_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            summary = campaign.run_campaign(Path(tmp) / "campaign27", repository_root=Path(tmp) / "knowledge_repository")
            final = (Path(tmp) / "campaign27" / "reports" / "campaign_27_final_report.md").read_text()
        ...
        self.assertIn("next approved WDI annual-scalar evidence family", summary["quality_report"]["final_recommendation"])
