...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign2_wdi_completeness_buckets.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign2_wdi_completeness_buckets", SCRIPT_PATH)
...
class Campaign2WdiCompletenessBucketTests(unittest.TestCase):
    def setUp(self):
        self.module = load_campaign_module()

    def test_snapshot_is_deterministic_and_bucketed(self):
        first = self.module.immutable_wdi_completeness_snapshot()
        second = self.module.immutable_wdi_completeness_snapshot()
        ...
        self.assertEqual(first["audited_inventory"]["territories_total"], 217)
        self.assertEqual(first["indicator_family_buckets"][0]["bucket_label"], "complete")
        self.assertEqual(first["period_buckets"][0]["bucket_label"], "complete")
        self.assertEqual(first["territory_buckets"][0]["bucket_label"], "complete")

    def test_campaign_produces_valid_completeness_bucket_objects(self):
        with tempfile.TemporaryDirectory() as td:
            output = Path(td) / "campaign2"
            ...
            self.assertIn("cross_campaign_metric_comparison", quality)
