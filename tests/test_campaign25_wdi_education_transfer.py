...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign25_wdi_education_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign25_wdi_education_transfer", SCRIPT_PATH)
...
class Campaign22WDIEducationTransferTests(unittest.TestCase):
    def test_campaign25_snapshot_is_education_annual_scalar_scope(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_education_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_education")
    ...
    def test_campaign25_builds_education_packages_without_boundary_terms(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_education_snapshot()
    ...
    def test_campaign25_run_is_deterministic_and_populates_repository_with_required_closeout(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign25"
            ...
            self.assertTrue((output / "reports" / "repository_education_summary.md").exists())
        ...
        self.assertEqual(quality1["repository_education_summary"]["total_knowledge_objects"], 19)
    ...
    def test_campaign25_reports_operational_expansion_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign25"
            repo = Path(tmp) / "knowledge_repository"
            summary = campaign.run_campaign(output, repository_root=repo)
            final = (output / "reports" / "campaign_25_final_report.md").read_text()
        ...
        self.assertIn("Education-specific", transfer)
        ...
        self.assertIn("Education", impact)
