...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign21_wdi_agriculture_rural_development_provenance_lineage_closeout.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign21_wdi_agriculture_rural_development_provenance_lineage_closeout", SCRIPT_PATH)
...
class Campaign21WDIAgricultureRuralDevelopmentProvenanceLineageCloseoutTests(unittest.TestCase):
    def test_snapshot_is_agriculture_provenance_lineage_scoped_and_architecture_preserving(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_agriculture_rural_development_provenance_lineage_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_agriculture_rural_development_provenance_lineage")
        self.assertEqual(snapshot["scope"]["domain_family"], "agriculture and rural development evidence")
        self.assertTrue(snapshot["agriculture_rural_development_family_maturity"]["classification"] == "Mature")
        self.assertFalse(snapshot["architectural_continuity_review"]["refinements_supported"])

    def test_campaign21_produces_agriculture_provenance_lineage_closeout_and_mature_family(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign21"
            ...
            family_report = (output / "reports" / "agriculture_rural_development_family_closeout_report.md").read_text()
        ...
        self.assertEqual(quality1["agriculture_rural_development_family_maturity_assessment"]["classification"], "Mature")
        ...
        self.assertIn("WDI Agriculture & Rural Development Production Family Closeout Report", family_report)
        self.assertIn("preserved across five Mature WDI annual-scalar production families", method_report)
    ...
    def test_final_recommendation_moves_to_next_roadmap_family_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            summary = campaign.run_campaign(Path(tmp) / "campaign21", repository_root=Path(tmp) / "knowledge_repository")
            final = (Path(tmp) / "campaign21" / "reports" / "campaign_21_final_report.md").read_text()
        ...
        self.assertIn("next Phase 2 production family", summary["quality_report"]["final_recommendation"])
