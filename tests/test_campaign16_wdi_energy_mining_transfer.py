...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign16_wdi_energy_mining_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign16_wdi_energy_mining_transfer", SCRIPT_PATH)
...
class Campaign16WDIEnergyMiningTransferTests(unittest.TestCase):
    def test_campaign16_snapshot_is_energy_mining_annual_scalar_scope(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_energy_mining_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_energy_mining")
        self.assertEqual(snapshot["scope"]["domain_family"], "energy and mining evidence")
        self.assertEqual(snapshot["audited_inventory"]["indicators_total"], 138)
        self.assertEqual(snapshot["scope"]["frequency"], "annual")
        self.assertEqual(snapshot["scope"]["shape"], "scalar observations")

    def test_campaign16_builds_energy_mining_packages_without_boundary_terms(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_energy_mining_snapshot()
    ...
    def test_campaign16_run_is_deterministic_and_populates_repository_with_required_closeout(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign16"
    ...
    def test_campaign16_reports_operational_expansion_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign16"
            repo = Path(tmp) / "knowledge_repository"
            summary = campaign.run_campaign(output, repository_root=repo)
            final = (output / "reports" / "campaign_16_final_report.md").read_text()
        ...
        self.assertIn("Energy & Mining-specific", transfer)
        ...
        self.assertIn("Energy & Mining", impact)
