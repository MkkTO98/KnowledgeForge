...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign20_wdi_agriculture_rural_development_maturation.py"
CAMPAIGN19_PATH = PROJECT_ROOT / "tools" / "run_campaign19_wdi_agriculture_rural_development_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign20_wdi_agriculture_rural_development_maturation", SCRIPT_PATH)
...
def load_campaign19_module():
    spec = importlib.util.spec_from_file_location("run_campaign19_wdi_agriculture_rural_development_transfer", CAMPAIGN19_PATH)
    if spec is None or spec.loader is None:
...
class Campaign20WDIAgricultureRuralDevelopmentMaturationTests(unittest.TestCase):
    def test_agriculture_maturation_snapshot_has_inventory_territorial_and_temporal_matrices(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_agriculture_rural_development_maturation_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_agriculture_rural_development_maturation")
        self.assertEqual(snapshot["scope"]["domain_family"], "agriculture and rural development evidence")
    ...
    def test_campaign20_generates_valid_agriculture_maturation_objects_and_health_summary(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_agriculture_rural_development_maturation_snapshot()
        ...
        self.assertIn("agriculture", promoted_text)

    def test_campaign20_is_deterministic_preserves_rejections_and_populates_repository_after_campaign19(self) -> None:
        campaign = load_campaign_module()
        campaign19 = load_campaign19_module()
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp) / "knowledge_repository"
            campaign19.run_campaign(Path(tmp) / "campaign19", repository_root=repo)
            output = Path(tmp) / "campaign20"
    ...
    def test_agriculture_stable_not_mature_and_doctrine_preserved(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            summary = campaign.run_campaign(Path(tmp) / "campaign20", repository_root=Path(tmp) / "knowledge_repository")
            quality = summary["quality_report"]
            final = (Path(tmp) / "campaign20" / "reports" / "campaign_20_final_report.md").read_text()
        self.assertEqual(quality["agriculture_rural_development_family_maturity_assessment"]["classification"], "Stable")
