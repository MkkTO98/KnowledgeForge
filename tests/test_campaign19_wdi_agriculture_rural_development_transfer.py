...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign19_wdi_agriculture_rural_development_transfer.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign19_wdi_agriculture_rural_development_transfer", SCRIPT_PATH)
...
class Campaign19WDIAgricultureRuralDevelopmentTransferTests(unittest.TestCase):
    def test_campaign19_snapshot_is_agriculture_rural_development_annual_scalar_scope(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_agriculture_rural_development_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_agriculture_rural_development")
        self.assertEqual(snapshot["scope"]["domain_family"], "agriculture and rural development evidence")
        self.assertEqual(snapshot["audited_inventory"]["indicators_total"], 126)
        self.assertEqual(snapshot["scope"]["frequency"], "annual")
        self.assertEqual(snapshot["scope"]["shape"], "scalar observations")

    def test_campaign19_builds_agriculture_packages_without_boundary_terms(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_agriculture_rural_development_snapshot()
    ...
    def test_campaign19_run_is_deterministic_and_populates_repository_with_required_closeout(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign19"
        ...
        self.assertEqual(quality1["repository_health_summary"]["total_knowledge_objects"], 19)
    ...
    def test_campaign19_reports_operational_expansion_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign19"
            repo = Path(tmp) / "knowledge_repository"
            summary = campaign.run_campaign(output, repository_root=repo)
            final = (output / "reports" / "campaign_19_final_report.md").read_text()
        ...
        self.assertIn("Agriculture & Rural Development-specific", transfer)
        ...
        self.assertIn("Agriculture & Rural Development", impact)
