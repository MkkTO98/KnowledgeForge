...
SCRIPT_PATH = PROJECT_ROOT / "tools" / "run_campaign24_wdi_health_provenance_lineage_closeout.py"


def load_campaign_module():
    spec = importlib.util.spec_from_file_location("run_campaign24_wdi_health_provenance_lineage_closeout", SCRIPT_PATH)
...
class Campaign24WDIHealthProvenanceLineageCloseoutTests(unittest.TestCase):
    def test_snapshot_is_health_provenance_lineage_scoped_and_architecture_preserving(self) -> None:
        campaign = load_campaign_module()
        snapshot = campaign.immutable_wdi_health_provenance_lineage_snapshot()
        self.assertEqual(snapshot["source_short_name"], "WDI")
        self.assertEqual(snapshot["evidence_family"], "external_wdi_annual_scalar_health_provenance_lineage")
        self.assertEqual(snapshot["scope"]["domain_family"], "health evidence")
        self.assertTrue(snapshot["health_family_maturity"]["classification"] == "Mature")
        self.assertFalse(snapshot["architectural_continuity_review"]["refinements_supported"])

    def test_campaign24_produces_health_provenance_lineage_closeout_and_mature_family(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "campaign24"
            ...
            family_report = (output / "reports" / "health_family_closeout_report.md").read_text()
        ...
        self.assertEqual(quality1["health_family_maturity_assessment"]["classification"], "Mature")
        ...
        self.assertIn("WDI Health Production Family Closeout Report", family_report)
        self.assertIn("preserved across six Mature WDI annual-scalar production families", method_report)
    ...
    def test_final_recommendation_moves_to_next_roadmap_family_without_architecture_change(self) -> None:
        campaign = load_campaign_module()
        with tempfile.TemporaryDirectory() as tmp:
            summary = campaign.run_campaign(Path(tmp) / "campaign24", repository_root=Path(tmp) / "knowledge_repository")
            final = (Path(tmp) / "campaign24" / "reports" / "campaign_24_final_report.md").read_text()
